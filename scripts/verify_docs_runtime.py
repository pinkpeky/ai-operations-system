...
class DocsRuntimeVerifier:
    ...
    def check_phase_status(self) -> None:
        ...
        for name, text in (("zh", zh_status), ("en", en_status)):
            if not re.search(r"Phase\s+31", text):
                self.error(f"{name}/PROJECT_STATUS.md missing Phase 31")
            elif "Worker Console Desktop App Foundation" not in text or "worker_console_desktop" not in text:
                self.error(f"{name}/PROJECT_STATUS.md does not describe Phase 31 scope")
            else:
                self.pass_(f"{name}/PROJECT_STATUS documents Phase 31")
