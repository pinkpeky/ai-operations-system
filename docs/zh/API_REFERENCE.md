...
## Phase 61Y: 商业运营 ComfyUI Runtime Dry-Runs

Phase 61Y 新增 `commercial_operation_comfyui_runtime_dry_runs` 与 `CommercialOperationComfyUIRuntimeDryRun` 记录，并提供 `/api/v1/commercial-operations/{operation_id}/comfyui-runtime-dry-runs`。操作人员和服务器维护人员可以从 armed ComfyUI runtime gate 创建 metadata-only runtime dry-run，编辑、送审、批准、驳回、标记 validated、标记 failed、取消或归档。该记录用于保存未来受控 ComfyUI adapter contract、dry-run request fixture、expected response contract、explicit server switch policy、validation checks、operator checklist 与 rollback plan；系统不会 import 或调用 ComfyUI adapter、不会保存或解析密钥值、不会请求 ComfyUI、不会提交 prompt、不会读取队列、不会上传文件、不会提交队列、不会启用 runtime switch、不会生成媒体。
...
Boundary: jobs must be approved and portrait consent must be authorized. `mock_render` writes a delivery manifest under `DIGITAL_HUMAN_OUTPUT_DIR`; `comfyui_handoff` creates a guarded `/api/v1/comfyui-runtime/video-jobs` record with GPU/queue admission, linked ComfyUI job id, progress, blocker reasons, and recovery details. Generated placeholder ComfyUI prompts are not submitted unless an operator supplies a real prompt and the existing guarded ComfyUI runtime gates permit submission. No publishing, account control, approval bypass, runtime mutation, service restart, model download, workflow installation, or package rebuild is added.

# Phase 67A Digital Human Foundation

Status: production foundation / provider execution disabled by default. Phase 67A adds a dedicated Digital Humans API surface for portrait/material upload and reviewable script-based video job plans. It does not call HeyGen, Tavus, D-ID, local MuseTalk/LivePortrait, ComfyUI workflows, OpenClaw/Playwright publishing, social platforms, or real accounts by default.
