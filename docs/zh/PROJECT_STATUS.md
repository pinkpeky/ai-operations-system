...
Current update (2026-05-22): PR #68 is the draft Phase 62B ComfyUI guarded read-only probe slice. The active next branch is `codex/phase-62c-comfyui-runtime-diagnostics`, scoped to Phase 62C ComfyUI Runtime Diagnostics: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `/api/v1/comfyui-runtime/diagnostics`, `ComfyUIRuntimeService`, `readiness_status`, `blocking_reasons`, `recommended_actions`, `read_only_probe_ready`, and a dedicated Admin Dashboard ComfyUI tab diagnostics panel. Diagnostics never attempts network calls. The only permitted live request remains `GET /system_stats` from health after every explicit provider/enabled/network/host/path/read-only gate is true. This phase does not import or call ComfyUI adapters, call execution endpoints, read queues, submit prompts or queues, upload files, generate media, enable runtime switches, mutate runtime configuration, store or resolve secrets, publish, control real accounts, ingest platform analytics, claim ROI attribution, or bypass approval.
