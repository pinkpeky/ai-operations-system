...
Current update (2026-05-25): active next branch `codex/phase-67a-digital-human-foundation` after Phase 66B ComfyUI Video Job Loop. It adds Phase 67A Digital Human Foundation: maintainers can upload consent-tracked portrait/material assets through `/api/v1/digital-humans/assets`, create script-based digital-human video jobs through `/api/v1/digital-humans/video-jobs`, refresh status through `/api/v1/digital-humans/video-jobs/{job_id}/refresh`, and approve/reject/cancel jobs through `/api/v1/digital-humans/video-jobs/{job_id}/{action}`. The Admin Dashboard now has a dedicated `digital-humans` page. Defaults remain mock/disabled with `DIGITAL_HUMAN_PROVIDER=mock`, `DIGITAL_HUMAN_ENABLED=false`, and `DIGITAL_HUMAN_ALLOW_EXTERNAL_API=false`, so HeyGen/Tavus/D-ID/local model execution is not called. It does not publish, control real accounts, bypass approval, mutate runtime configuration, restart services, download models, install workflows, or rebuild client packages.
