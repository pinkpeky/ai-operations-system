...
Current update (2026-05-21): PR #66 is the draft Phase 61Z ComfyUI runtime activation slice. The active next branch is `codex/phase-62a-comfyui-runtime-adapter-contract`, scoped to Phase 62A ComfyUI Runtime Adapter Contract: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `ComfyUIRuntimeService`, `COMFYUI_RUNTIME_*` settings, Docker environment exposure, docs/runtime verification, and a dedicated Admin Dashboard ComfyUI tab contract panel. This phase does not import or call ComfyUI adapters, call ComfyUI endpoints, read queues, submit prompts or queues, upload files, generate media, enable runtime switches, mutate runtime configuration, store or resolve secrets, publish, control real accounts, ingest platform analytics, claim ROI attribution, or bypass approval.
