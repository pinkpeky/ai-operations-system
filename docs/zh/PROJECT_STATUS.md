...
Current update (2026-05-22): PR #69 is the draft Phase 62C ComfyUI runtime diagnostics slice. The active next branch is `codex/phase-62d-comfyui-runtime-diagnostic-snapshots`, scoped to Phase 62D ComfyUI Runtime Diagnostic Snapshots: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `/api/v1/comfyui-runtime/diagnostics`, `POST /api/v1/comfyui-runtime/diagnostic-snapshots`, `GET /api/v1/comfyui-runtime/diagnostic-snapshots`, `comfyui_runtime_diagnostic_snapshots`, `ComfyUIRuntimeService`, `readiness_status`, `blocking_reasons`, `recommended_actions`, `read_only_probe_ready`, operator notes, snapshot metadata, and a dedicated Admin Dashboard ComfyUI tab snapshot action. Diagnostic snapshots never attempt network calls. The only permitted live request remains `GET /system_stats` from health after every explicit provider/enabled/network/host/path/read-only gate is true. This phase does not import or call ComfyUI adapters, call execution endpoints, read queues, submit prompts or queues, upload files, generate media, enable runtime switches, mutate runtime configuration, store or resolve secrets, publish, control real accounts, ingest platform analytics, claim ROI attribution, or bypass approval.
