...
Current update (2026-05-22): active next branch `codex/phase-62m-client-goal-templates` and PR #79 draft for Phase 62M Customer Console Goal Templates after PR #78 / Phase 62L Customer Console Task Workbench. It adds standard launch content, RAG evidence, asset brief, and page report templates to the first `worker_console` and `worker_console_desktop` task workbench, so customer-machine operators can start from a clear operating goal and recommended playbook. It preserves local runtime/heartbeat controls, Chinese/English language switching, approvals, playbooks, outputs, tasks, logs, and server/client boundary warnings. It does not import ComfyUI adapters, call ComfyUI, submit prompts, read or submit queues, upload files, generate media, enable runtime switches, mutate runtime configuration, resolve secrets, publish, run OpenClaw, control accounts, or bypass approval.
