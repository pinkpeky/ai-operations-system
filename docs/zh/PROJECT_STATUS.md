...
Current update (2026-05-25): active next branch `codex/phase-66b-comfyui-video-job-loop` after Phase 66A ComfyUI Video Resource Line. It adds Phase 66B ComfyUI Video Job Loop: maintainers can create persisted video jobs through `POST /api/v1/comfyui-runtime/video-jobs`, reuse Phase 66A GPU/queue admission before real `/prompt`, store `runtime_prompt_id`, `runtime_base_url`, `resource_plan`, `selected_endpoint`, `selected_gpu`, queue/history payloads, output files, `job_status`, `failure_reason`, and `result_summary`, then recover progress through `POST /api/v1/comfyui-runtime/video-jobs/{job_id}/refresh`. Defaults still require every Phase 65A/65B runtime gate plus the Phase 66A video resource limits. It does not upload files, download models, resolve secrets, publish to social media, control real accounts, bypass approval, mutate runtime configuration, restart services, or rebuild client packages.
