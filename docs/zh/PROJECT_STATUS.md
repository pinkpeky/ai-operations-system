...
Current update (2026-05-25): active next branch `codex/phase-67b-digital-human-execution-loop` after Phase 67A Digital Human Foundation. It adds Phase 67B Digital Human Execution Loop: approved digital-human video jobs can call `/api/v1/digital-humans/video-jobs/{job_id}/execute` to create either a local delivery manifest asset under `DIGITAL_HUMAN_OUTPUT_DIR` or a guarded ComfyUI video handoff linked to `/api/v1/comfyui-runtime/video-jobs`. The Admin Dashboard `digital-humans` page now shows progress, delivery asset creation, and ComfyUI handoff controls; `worker_console` and `worker_console_desktop` show a compact digital-human video progress card for customer-machine operators. Defaults remain mock/disabled with `DIGITAL_HUMAN_PROVIDER=mock`, `DIGITAL_HUMAN_ENABLED=false`, and `DIGITAL_HUMAN_ALLOW_EXTERNAL_API=false`, and ComfyUI prompt submission still requires the existing guarded runtime gates. It does not call HeyGen/Tavus/D-ID/local avatar models by default, publish, control real accounts, bypass approval, mutate runtime configuration, restart services, download models, install workflows, or rebuild client packages.
