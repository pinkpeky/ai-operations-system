...
The `ComfyUIRuntimeService` now can call only guarded ComfyUI health, prompt, history, and queue endpoints after every explicit gate is enabled. It still does not upload files, submit arbitrary queue jobs outside the allowlist, resolve secret values, enable runtime switches, mutate environment/config, publish, run OpenClaw, control Browser Worker actions, contact external accounts, or bypass approval. The Admin Dashboard exposes the contract, runbook, snapshots, configuration change requests, manual apply evidence, post-manual readiness checks, guarded probe execution, real queue status, and smoke prompt result in the dedicated `?page=comfyui-operations` tab.
