...
The next active branch is `codex/phase-63g-next-cycle-execution-prep`. Its scope is Phase 63G Customer Console Next-Cycle Execution Prep: `worker_console` and `worker_console_desktop` can recognize pending next-cycle approvals, approve or reject them with next-cycle wording, package the next-cycle content draft, and create a metadata-only execution prep request for the second iteration. It keeps the repeatable closed loop moving from second draft into execution preparation without running OpenClaw, Playwright, publishing, account control, ComfyUI, analytics ingestion, captcha/proxy/fingerprint bypass, secret resolution, or approval bypass. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.

The direct predecessor chain is Phase 63B first draft bootstrap, then Phase 63C approval and execution prep, Phase 63D execution run review, Phase 63E result feedback loop with the minimum usable closed loop, Phase 63F next-cycle content draft generation, and Phase 63G next-cycle execution prep.
