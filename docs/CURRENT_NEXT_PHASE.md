...
`codex/phase-63a-client-loop-protocol-binding`
...
The next active branch is `codex/phase-63a-client-loop-protocol-binding`. Its scope is Phase 63A Customer Console Loop Protocol Binding: `worker_console` and `worker_console_desktop` now call the commercial operation APIs, can create a customer-facing operation from the current goal, refresh `GET /api/v1/commercial-operations/{operation_id}/operation-loop`, and render the real operation-loop status before falling back to local task state. It keeps the customer-machine surface simple: one goal, one visible loop, create/refresh, interrupt/continue, output review, and knowledge upload access. It does not execute OpenClaw, run Playwright, publish to social media, control real accounts, call ComfyUI, bypass captcha, use proxy pools, bypass fingerprints, resolve secrets, or bypass approval. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.
