...
1. Review and keep the Phase 62G stacked draft PR from `codex/phase-62g-comfyui-manual-apply-evidence` aligned above PR #72.
2. Start Phase 62H as a guarded post-manual readiness comparison slice: compare approved request data, manual apply evidence, before/after snapshots, and diagnostics before any live ComfyUI health probe or adapter execution is allowed.
...
4. Continue toward guarded ComfyUI/OpenClaw adapters, richer knowledge upload ergonomics, monitoring metrics, failure recovery, and commercial result reporting after the manual-apply audit loop is reviewable.
