...
The next active branch is `codex/phase-67e-digital-human-output-ingestion`. Its scope is Phase 67E Digital Human Output Ingestion: keep the Phase 67A `digital_human_assets` / `digital_human_video_jobs` foundation with `/api/v1/digital-humans/capabilities`, `/api/v1/digital-humans/assets`, and `/api/v1/digital-humans/video-jobs`, keep the Phase 67B `/api/v1/digital-humans/video-jobs/{job_id}/execute` handoff, keep the Phase 67C `/api/v1/digital-humans/workflow-templates`, `/api/v1/digital-humans/workflow-templates/{template_id}`, and `/api/v1/digital-humans/video-jobs/{job_id}/workflow-binding` contract layer, keep Phase 67D `/api/v1/digital-humans/video-jobs/{job_id}/workflow-readiness-check`, then add `/api/v1/digital-humans/video-jobs/{job_id}/comfyui-output-ingestion`. The new ingestion endpoint refreshes the linked ComfyUI video job, reads recorded outputs, registers generated video assets, and exposes `comfyui_output_ingestion_status`, `delivery_asset_id`, `delivery_asset_status`, `delivery_source_uri`, and `delivery_output_count`; Admin Dashboard exposes these as operator controls and customer consoles show ingestion/delivery status. Defaults are `DIGITAL_HUMAN_PROVIDER=mock`, `DIGITAL_HUMAN_ENABLED=false`, and `DIGITAL_HUMAN_ALLOW_EXTERNAL_API=false`, and ingestion does not resubmit ComfyUI jobs unless `resubmit_if_waiting` is explicitly enabled. No HeyGen, Tavus, D-ID, local MuseTalk/LivePortrait execution, workflow installation automation, model download automation, automatic ComfyUI file upload, social publishing, account control, runtime configuration mutation, service restart, approval bypass, or client package rebuild is added. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.

The direct predecessor chain is Phase 63B first draft bootstrap, then Phase 63C approval and execution prep, Phase 63D execution run review, Phase 63E result feedback loop with the minimum usable closed loop, Phase 63F next-cycle content draft generation, Phase 63G next-cycle execution prep, Phase 63H next-cycle execution run review, Phase 63I next-cycle result feedback, Phase 63J client runtime preflight, Phase 63K guarded adapter dispatch handoff, Phase 63L-63N execution and approval loop with guarded adapter dry-run, client execution queue, and commercial approval center, Phase 63O-63Q publish result observation loop, Phase 63R-63T publish metric improvement loop with manual publish metric improvement and publish metric next-cycle draft preparation, Phase 63U-63W improved draft re-execution loop with publish metric re-execution prep, Phase 63X-64B client closed-loop delivery pass, Phase 64C Commercial Agent/Skill Orchestration, Phase 64D Server/Client Frontend Operability Optimization, Phase 64E Layout Declutter, Phase 65A ComfyUI Real Adapter, Phase 65B Commercial ComfyUI Execution Link, Phase 66A ComfyUI Video Resource Line, Phase 66B ComfyUI Video Job Loop, Phase 67A Digital Human Foundation, Phase 67B Digital Human Execution Loop, Phase 67C Digital Human Workflow Binding, Phase 67D Digital Human Workflow Readiness, and Phase 67E Digital Human Output Ingestion.
...
Phase 67A Digital Human Foundation on `codex/phase-67a-digital-human-foundation` adds consent-tracked digital human asset uploads and reviewable video job plans. Server maintainers can upload authorized portraits and supporting materials, create a script-based digital-human video job, refresh its status, approve/reject/cancel it, and inspect provider readiness from the dedicated Admin Dashboard Digital Humans page. Boundaries remain explicit: defaults are mock/disabled and it does not call HeyGen, Tavus, D-ID, local MuseTalk/LivePortrait, ComfyUI workflows, publish, execute OpenClaw/Playwright account actions, mutate runtime configuration, restart services, bypass approvals, or rebuild packages.
