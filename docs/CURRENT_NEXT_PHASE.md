...
`codex/phase-62n-client-goal-plan-preview`
...
The next active branch is `codex/phase-62n-client-goal-plan-preview`. Its scope is Phase 62N Customer Console Goal Plan Preview: it adds compact plan previews to the selected customer-machine goal template in `worker_console` and `worker_console_desktop`, so operators can see planned steps, approval boundary, and expected output before running a goal. It does not add ComfyUI calls, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha bypass, proxy pools, fingerprint bypass, secret resolution, or approval bypass. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.
...
2. Review Phase 62N customer console goal plan previews above PR #79 and verify customer-machine operators can understand planned steps, approval boundary, and expected output before running a selected goal.
