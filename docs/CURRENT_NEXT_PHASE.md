...
`codex/phase-66b-comfyui-video-job-loop`
...
Current effective phase: Phase 66B ComfyUI Video Job Loop. The old reverted Phase 56 branch remains inactive and is not a valid continuation point.

## Current Recommended Next Phase

Readiness and PR cleanup are complete. PR #57 through PR #112 remain the stacked commercial/ComfyUI/customer-console draft chain through Phase 66A. The active local branch after PR #112 is `codex/phase-66b-comfyui-video-job-loop`, scoped to Phase 66B ComfyUI Video Job Loop.
...
The next active branch is `codex/phase-66b-comfyui-video-job-loop`. Its scope is Phase 66B ComfyUI Video Job Loop: add persisted `POST /api/v1/comfyui-runtime/video-jobs`, `GET /api/v1/comfyui-runtime/video-jobs`, `GET /api/v1/comfyui-runtime/video-jobs/{job_id}`, and `POST /api/v1/comfyui-runtime/video-jobs/{job_id}/refresh`; reuse Phase 66A GPU/queue admission before real `/prompt`; store `runtime_prompt_id`, `runtime_base_url`, `resource_plan`, `selected_endpoint`, `selected_gpu`, queue/history payloads, output files, `job_status`, `failure_reason`, and `result_summary`; and expose concise Admin Dashboard video job controls. Defaults still depend on every Phase 65A/65B runtime gate and Phase 66A resource gate, and no file upload, model download automation, workflow installation, secret resolution, social publishing, account control, runtime configuration mutation, service restart, approval bypass, or client package rebuild is added. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.

The direct predecessor chain is Phase 63B first draft bootstrap, then Phase 63C approval and execution prep, Phase 63D execution run review, Phase 63E result feedback loop with the minimum usable closed loop, Phase 63F next-cycle content draft generation, Phase 63G next-cycle execution prep, Phase 63H next-cycle execution run review, Phase 63I next-cycle result feedback, Phase 63J client runtime preflight, Phase 63K guarded adapter dispatch handoff, Phase 63L-63N execution and approval loop with guarded adapter dry-run, client execution queue, and commercial approval center, Phase 63O-63Q publish result observation loop, Phase 63R-63T publish metric improvement loop with manual publish metric improvement and publish metric next-cycle draft preparation, Phase 63U-63W improved draft re-execution loop with publish metric re-execution prep, Phase 63X-64B client closed-loop delivery pass, Phase 64C Commercial Agent/Skill Orchestration, Phase 64D Server/Client Frontend Operability Optimization, Phase 64E Layout Declutter, Phase 65A ComfyUI Real Adapter, Phase 65B Commercial ComfyUI Execution Link, Phase 66A ComfyUI Video Resource Line, and Phase 66B ComfyUI Video Job Loop.
