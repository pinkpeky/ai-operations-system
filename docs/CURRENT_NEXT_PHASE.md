...
`codex/phase-63b-client-first-draft-bootstrap`
...
The next active branch is `codex/phase-63b-client-first-draft-bootstrap`. Its scope is Phase 63B Customer Console First Draft Bootstrap: `worker_console` and `worker_console_desktop` now use the commercial operation APIs to regenerate a plan, create a first content draft, mark it ready for review, create a human approval gate, and refresh `operation-loop` from one simple customer-machine action. It keeps the first usable closed loop moving from goal to first draft without running OpenClaw, Playwright, publishing, account control, ComfyUI, captcha/proxy/fingerprint bypass, secret resolution, or approval bypass. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.
