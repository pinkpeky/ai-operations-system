...
The next active branch is `codex/phase-57-run-cockpit-closeout`. Its scope is reconciling Phase 57 run cockpit documentation after PR #24, recording the completed operator-controls slice, and adding a lightweight status guard so merged run cockpit slices do not remain marked active. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.
...
1. Finish Phase 57 Run Cockpit Closeout & Docs Reconciliation on `codex/phase-57-run-cockpit-closeout`.
2. Verify docs runtime checks and the focused run cockpit documentation guard.
