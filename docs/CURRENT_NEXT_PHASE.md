...
The next active branch is `codex/phase-62c-comfyui-runtime-diagnostics`. Its scope is adding `GET /api/v1/comfyui-runtime/diagnostics`: a no-network readiness report for `provider_guarded`, `runtime_enabled`, `network_gate`, `base_url_scheme`, `base_url_host_allowlist`, `read_only_probe_gate`, `health_path_allowlist`, and `execution_boundary`. It returns `readiness_status`, `blocking_reasons`, `recommended_actions`, `read_only_probe_ready`, and `diagnostic_checks` for server maintainers, while `GET /api/v1/comfyui-runtime/health` remains the only endpoint that may attempt the guarded `GET /system_stats` probe after every explicit Phase 62B gate is enabled. It still does not import or call a ComfyUI adapter, call execution endpoints, read ComfyUI queues, submit prompts or queues, upload files to ComfyUI, generate images/videos, store or resolve secret values, enable runtime server switches, mutate runtime configuration, upload or ingest new knowledge files, auto-approve assets, auto-publish, control real accounts, execute OpenClaw actions, run Browser Worker actions, ingest platform analytics, claim ROI attribution, or bypass approval. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.
...
1. Finish the Phase 62C ComfyUI runtime diagnostics backend/API/frontend/docs slice.
2. Run backend, frontend, docs, migration, and browser verification gates.
3. Keep PR #68 as the Phase 62B draft and open/update the Phase 62C stacked draft PR from `codex/phase-62c-comfyui-runtime-diagnostics` when verification passes.
