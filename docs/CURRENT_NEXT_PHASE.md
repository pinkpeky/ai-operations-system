...
`codex/phase-62u-knowledge-ingestion-status`
...
The next active branch is `codex/phase-62u-knowledge-ingestion-status`. Its scope is Phase 62U Customer Console Knowledge Ingestion Status Loop: it builds on the Phase 62T visual knowledge search validation page by adding a customer-machine ingestion status loop for `worker_console` and `worker_console_desktop`. Operators can see queued/uploading/processing/search-ready/needs-review counts, a simple step pipeline, latest upload results, source/document ids, chunk counts, duplicate-skip notes, failure reasons, refresh, retry, and selected-document ingestion status without reading code or JSON. The knowledge ingestion status loop stays visual and frontend-focused, using existing file upload, document list, text ingest, reingest, and RAG search APIs; it does not add ComfyUI calls, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha bypass, proxy pools, fingerprint bypass, secret resolution, or approval bypass. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.
...
8. Add monitored analytics/result feedback adapters only after the probe execution audit trail is reviewable and failure recovery is visible to server maintainers.
9. Keep Docker compose running only while manual inspection is useful; otherwise shut it down cleanly with `docker compose -f docker-compose.yml down`.
10. Continue toward guarded ComfyUI/OpenClaw adapters, richer knowledge upload ergonomics, monitoring metrics, failure recovery, and commercial result reporting after the probe audit loop is reviewable.
