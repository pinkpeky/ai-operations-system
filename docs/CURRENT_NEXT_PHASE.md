...
Current effective phase: Phase 58 Run Cockpit Playbook Context. The old reverted Phase 56 branch remains inactive and is not a valid continuation point.
...
The next active branch is `codex/phase-58-playbook-thread-context`. Its scope is completing the Run Cockpit-to-Playbooks handoff by applying `thread_id` context to Playbook Runs, adding linked-conversation navigation, and adding a clear-context control. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.
...
The active pull request for Phase 58C is not opened yet at the time of this update.
...
1. Finish Phase 58C Run Cockpit Playbook Thread Context on `codex/phase-58-playbook-thread-context`.
