...
`codex/phase-62p-client-simple-operator-mode`
...
The next active branch is `codex/phase-62p-client-simple-operator-mode`. Its scope is Phase 62P Customer Console Simple Operator Mode: it makes `worker_console` and `worker_console_desktop` default to a simple operator surface with goal input, common task chips, progress visibility, collapsed maintenance details, and a separate visual knowledge base upload/edit page for uploading files, adding text material, updating existing sources, and refreshing document cards. It does not show code or JSON on the knowledge page, and it does not add ComfyUI calls, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha bypass, proxy pools, fingerprint bypass, secret resolution, or approval bypass. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.
...
2. Review the active Phase 62P Customer Console Simple Operator Mode branch after PR #81 and verify customer-machine operators can use the simple task page plus the knowledge base upload/edit page without opening advanced diagnostics or reading raw payloads.
