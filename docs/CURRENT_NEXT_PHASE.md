...
The next active branch is `codex/phase-62h-comfyui-post-manual-readiness`. Its scope is adding `comfyui_runtime_post_manual_readiness_checks`, `ComfyUIRuntimePostManualReadinessCheck`, `POST /api/v1/comfyui-runtime/manual-apply-evidence/{evidence_id}/post-manual-readiness-checks`, `GET /api/v1/comfyui-runtime/post-manual-readiness-checks`, and ready/approve/reject/fail/archive review actions for metadata-only post-manual readiness comparisons. A check can only be created from verified Phase 62G evidence. It compares before/after/current readiness, current no-network diagnostics, manual apply evidence, restart evidence, `guarded_probe_ready`, `comparison_status`, `health_probe_executed=false`, `external_request_attempted=false`, `runtime_calls_enabled=false`, and `api_config_mutation_performed=false`. The Admin Dashboard ComfyUI page loads these records and exposes create/review controls for server maintainers. It still does not import or call a ComfyUI adapter, call execution endpoints, read ComfyUI queues, submit prompts or queues, upload files to ComfyUI, generate images/videos, store or resolve secret values, enable runtime server switches, mutate runtime configuration, write environment variables, restart services, run `/system_stats`, upload or ingest new knowledge files, auto-approve assets, auto-publish, control real accounts, execute OpenClaw actions, run Browser Worker actions, ingest platform analytics, claim ROI attribution, or bypass approval. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.

Planned follow-up: insert Phase 62I Workstation/Customer Client Frontend UX Alignment before any additional live ComfyUI probe escalation. Phase 62I should turn `worker_console` and `worker_console_desktop` into a clearer customer-machine operator surface: simple connection status, local runtime/heartbeat controls, conversation/playbook/task/output shortcuts, approval queue visibility, failure recovery guidance, setup/help panels, Chinese/English language switching, and explicit server-vs-customer-machine boundary warnings. It should not add real ComfyUI calls, OpenClaw execution, platform publishing, account control, captcha/proxy/fingerprint bypass, or production installer/signing. The guarded read-only probe approval/execution slice moves after that client UX alignment.
...
1. Review and keep the Phase 62H stacked draft PR from `codex/phase-62h-comfyui-post-manual-readiness` aligned above PR #73.
2. Start Phase 62I as Workstation/Customer Client Frontend UX Alignment: simplify `worker_console` and `worker_console_desktop`, add role-appropriate operator flows, language switching, setup/help, and failure recovery guidance.
3. Start the guarded read-only probe approval/execution slice only after the client UX alignment is in place and readiness checks explicitly show `guarded_probe_ready=true`.
...
5. Continue toward guarded ComfyUI/OpenClaw adapters, richer knowledge upload ergonomics, monitoring metrics, failure recovery, and commercial result reporting after the manual-apply audit loop and workstation UX are reviewable.
