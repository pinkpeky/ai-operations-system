...
Current effective phase: Phase 67A Digital Human Foundation. The old reverted Phase 56 branch remains inactive and is not a valid continuation point.

## Current Recommended Next Phase

Readiness and PR cleanup are complete. PR #57 through PR #112 remain the stacked commercial/ComfyUI/customer-console draft chain through Phase 66A, Phase 66B added the recoverable video job loop, and PR #114 is the draft Phase 67A Digital Human Foundation branch. The active local branch after Phase 66B is `codex/phase-67a-digital-human-foundation`, scoped to Phase 67A Digital Human Foundation.
...
The next active branch is `codex/phase-67a-digital-human-foundation`. Its scope is Phase 67A Digital Human Foundation: add `digital_human_assets`, `digital_human_video_jobs`, `/api/v1/digital-humans/capabilities`, `/api/v1/digital-humans/assets`, `/api/v1/digital-humans/video-jobs`, `/api/v1/digital-humans/video-jobs/{job_id}/refresh`, and `/api/v1/digital-humans/video-jobs/{job_id}/{action}`; support consent-tracked portrait/material uploads; create script-based digital-human video job plans; expose approval status, consent status, provider boundary fields, and a dedicated Admin Dashboard Digital Humans page. Defaults are `DIGITAL_HUMAN_PROVIDER=mock`, `DIGITAL_HUMAN_ENABLED=false`, and `DIGITAL_HUMAN_ALLOW_EXTERNAL_API=false`, so no HeyGen, Tavus, D-ID, local MuseTalk/LivePortrait, ComfyUI workflow, social publishing, account control, runtime configuration mutation, service restart, approval bypass, or client package rebuild is added. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.

The direct predecessor chain is Phase 63B first draft bootstrap, then Phase 63C approval and execution prep, Phase 63D execution run review, Phase 63E result feedback loop with the minimum usable closed loop, Phase 63F next-cycle content draft generation, Phase 63G next-cycle execution prep, Phase 63H next-cycle execution run review, Phase 63I next-cycle result feedback, Phase 63J client runtime preflight, Phase 63K guarded adapter dispatch handoff, Phase 63L-63N execution and approval loop with guarded adapter dry-run, client execution queue, and commercial approval center, Phase 63O-63Q publish result observation loop, Phase 63R-63T publish metric improvement loop with manual publish metric improvement and publish metric next-cycle draft preparation, Phase 63U-63W improved draft re-execution loop with publish metric re-execution prep, Phase 63X-64B client closed-loop delivery pass, Phase 64C Commercial Agent/Skill Orchestration, Phase 64D Server/Client Frontend Operability Optimization, Phase 64E Layout Declutter, Phase 65A ComfyUI Real Adapter, Phase 65B Commercial ComfyUI Execution Link, Phase 66A ComfyUI Video Resource Line, Phase 66B ComfyUI Video Job Loop, and Phase 67A Digital Human Foundation.
