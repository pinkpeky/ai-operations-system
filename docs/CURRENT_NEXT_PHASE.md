...
The next active branch is `codex/phase-62b-comfyui-guarded-readonly-probe`. Its scope is adding a guarded read-only ComfyUI health probe to `GET /api/v1/comfyui-runtime/health`: default no-network behavior, `COMFYUI_RUNTIME_READ_ONLY_PROBE_ENABLED`, `COMFYUI_RUNTIME_HEALTH_PATH`, `COMFYUI_RUNTIME_ALLOWED_HEALTH_PATHS`, host/path allowlist checks, response status/latency visibility, Docker environment exposure, dedicated Admin Dashboard ComfyUI tab probe visibility, and docs/runtime verification. It can only attempt `GET /system_stats` when every explicit gate is enabled. It still does not import or call a ComfyUI adapter, call execution endpoints, read ComfyUI queues, submit prompts or queues, upload files to ComfyUI, generate images/videos, store or resolve secret values, enable runtime server switches, mutate runtime configuration, upload or ingest new knowledge files, auto-approve assets, auto-publish, control real accounts, execute OpenClaw actions, run Browser Worker actions, ingest platform analytics, claim ROI attribution, or bypass approval. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.
...
1. Finish the Phase 62B ComfyUI guarded read-only probe backend/API/frontend/docs slice.
2. Run backend, frontend, docs, migration, and browser verification gates.
3. Keep PR #67 as the Phase 62A draft and open/update the Phase 62B stacked draft PR from `codex/phase-62b-comfyui-guarded-readonly-probe` when verification passes.
