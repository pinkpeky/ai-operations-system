...
`codex/phase-62t-knowledge-search-validation`
...
The next active branch is `codex/phase-62t-knowledge-search-validation`. Its scope is Phase 62T Customer Console Knowledge Search Validation: it builds on the Phase 62S visual knowledge document details page by adding customer-machine search validation against the existing `/rag/search` API for `worker_console` and `worker_console_desktop`. Operators can enter a validation question, choose hybrid/semantic/keyword search, see matched source snippets, source labels, chunk indexes, scores, empty-result guidance, and a local activity record. The knowledge search validation flow stays visual and does not show code or JSON on the knowledge page, and it does not add ComfyUI calls, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha bypass, proxy pools, fingerprint bypass, secret resolution, or approval bypass. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.
...
7. Add monitored analytics/result feedback adapters only after the probe execution audit trail is reviewable and failure recovery is visible to server maintainers.
8. Keep Docker compose running only while manual inspection is useful; otherwise shut it down cleanly with `docker compose -f docker-compose.yml down`.
9. Continue toward guarded ComfyUI/OpenClaw adapters, richer knowledge upload ergonomics, monitoring metrics, failure recovery, and commercial result reporting after the probe audit loop is reviewable.
