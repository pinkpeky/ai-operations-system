...
`codex/phase-62k-customer-console-codex-ux`
...
The next active branch is `codex/phase-62k-customer-console-codex-ux`. Its scope is Phase 62K Customer Console Codex-like UX Simplification: it refactors `worker_console` and `worker_console_desktop` so the first screen is a concise customer-machine control surface with a left status rail, central next-step guidance, a prominent conversation command input, and advanced maintenance/diagnostics collapsed behind a disclosure. It does not add ComfyUI calls, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha bypass, proxy pools, fingerprint bypass, secret resolution, or approval bypass. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.
...
2. Review Phase 62K customer console UX above PR #76 and verify customer-machine operators see a simple command surface before advanced diagnostics.
