...
`codex/phase-63d-client-execution-run-review`
...
The next active branch is `codex/phase-63d-client-execution-run-review`. Its scope is Phase 63D Customer Console Execution Run Review: `worker_console` and `worker_console_desktop` can review the metadata-only execution prep request, approve and prepare it, create an execution run record, mark that run started, record a failure, and retry a failed run from one simple customer-machine operation desk. It keeps the first usable closed loop moving from execution prep into recoverable execution-run tracking without running OpenClaw, Playwright, publishing, account control, ComfyUI, captcha/proxy/fingerprint bypass, secret resolution, or approval bypass. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.

The direct predecessor chain is Phase 63B first draft bootstrap, then Phase 63C approval and execution prep, then Phase 63D execution run review.
