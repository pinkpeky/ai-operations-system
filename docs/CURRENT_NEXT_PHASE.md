...
The next active branch is `codex/phase-61n-commercial-rag-evidence-generation`. Its scope is adding `/api/v1/commercial-operations/{operation_id}/evidence-snapshots/generate-rag`, retrieved RAG chunk evidence items, source document IDs, search metadata, and an Admin Dashboard action that creates draft evidence snapshots from existing RAG search. It still creates traceable draft records only; it does not upload or ingest new knowledge files, auto-approve evidence, auto-publish, control real accounts, execute OpenClaw actions, run Browser Worker actions, run ComfyUI jobs, ingest platform analytics, claim ROI attribution, or bypass approval. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.
...
| #54 | Phase 61N Commercial Operation RAG Evidence Generation | `codex/phase-61n-commercial-rag-evidence-generation` | Draft PR |

PR #53 was merged to `main`; Phase 61N is the current development slice and will open as PR #54 from `codex/phase-61n-commercial-rag-evidence-generation`.
...
- Phase 57A-57D, Phase 58A-58E, Phase 59A-59C, Phase 60A-60G, and Phase 61A-61M are present on `main` through PR #22-#53.
...
1. Finish the Phase 61N commercial RAG evidence generation backend/API/frontend/docs slice.
2. Run backend, frontend, docs, migration, and browser verification gates.
3. Open PR #54 as a draft from `codex/phase-61n-commercial-rag-evidence-generation`.
