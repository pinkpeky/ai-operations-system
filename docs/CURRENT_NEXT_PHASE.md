...
`codex/phase-62v-knowledge-validation-guidance`
...
The next active branch is `codex/phase-62v-knowledge-validation-guidance`. Its scope is Phase 62V Customer Console Knowledge Validation Guidance: it builds on the Phase 62U ingestion status loop by adding suggested validation questions, selected-document validation, latest-upload validation, and one-click question fill/run actions for `worker_console` and `worker_console_desktop`. Operators can upload or select material, then validate core content, risk/limitations, and execution notes without inventing search terms or reading code/JSON. The knowledge validation guidance stays visual and frontend-focused, using existing file upload, document list, text ingest, reingest, and RAG search APIs; it does not add ComfyUI calls, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha bypass, proxy pools, fingerprint bypass, secret resolution, or approval bypass. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.
...
8. Finish Phase 62V Customer Console Knowledge Validation Guidance on `codex/phase-62v-knowledge-validation-guidance` and verify operators can validate selected or newly uploaded material with suggested questions and one-click RAG search without reading code or JSON.
9. Add monitored analytics/result feedback adapters only after the probe execution audit trail is reviewable and failure recovery is visible to server maintainers.
10. Keep Docker compose running only while manual inspection is useful; otherwise shut it down cleanly with `docker compose -f docker-compose.yml down`.
11. Continue toward guarded ComfyUI/OpenClaw adapters, richer knowledge upload ergonomics, monitoring metrics, failure recovery, and commercial result reporting after the probe audit loop is reviewable.
