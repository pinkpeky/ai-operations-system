...
The next active branch is `codex/phase-62i-workstation-client-ux`. Its scope is Phase 62I Workstation/Customer Client Frontend UX Alignment: `worker_console` and `worker_console_desktop` now prioritize a clearer customer-machine operator surface with simple connection status, local runtime/heartbeat controls, conversation/playbook/task/output shortcuts, approval queue visibility, failure recovery guidance, setup/help panels, Chinese/English language switching, and explicit server-vs-customer-machine boundary warnings. It still does not add real ComfyUI calls, OpenClaw execution, platform publishing, account control, captcha/proxy/fingerprint bypass, production installer/signing, auto-update, secret resolution, or approval bypass. CI readiness tracking lives in `docs/CI_READINESS_GATES.md`; branch protection guidance lives in `docs/BRANCH_PROTECTION.md`; scheduled smoke guidance lives in `docs/SCHEDULED_SMOKE.md`; run cockpit guidance lives in `docs/RUN_COCKPIT_FOUNDATION.md`.
...
2. Finish Phase 62I Workstation/Customer Client Frontend UX Alignment, including Web Worker Console and Tauri Desktop Console verification.
3. Start the guarded read-only probe approval/execution slice only after the client UX alignment is reviewable and readiness checks explicitly show `guarded_probe_ready=true`.
