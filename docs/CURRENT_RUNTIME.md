...
Phase 67C adds Digital Human Workflow Binding. `GET /api/v1/digital-humans/workflow-templates` and `GET /api/v1/digital-humans/workflow-templates/{template_id}` expose operator-verifiable ComfyUI workflow contracts such as `liveportrait-musetalk-broll`, including required assets, nodes, models, input slots, output slots, guardrails, and resource hints. `POST /api/v1/digital-humans/video-jobs/{job_id}/workflow-binding` binds an authorized portrait plus selected material/reference assets into a reviewable ComfyUI input contract and records `selected_workflow_template_id`, `workflow_binding_status`, missing inputs, node/model requirements, and a guarded prompt/workflow contract. Bound contracts can be handed to Phase 67B `comfyui_handoff`, but submission still requires an operator-supplied real prompt plus the existing guarded ComfyUI runtime gates. Phase 67C does not install workflows, download models, upload files to ComfyUI, mutate runtime configuration, restart services, publish, execute OpenClaw/Playwright account actions, control accounts, ingest analytics, bypass approvals, or rebuild packages.
...
| `DIGITAL_HUMAN_PROVIDER` | `mock` | Digital human provider selector. Current Phase 67C behavior supports local delivery manifests, guarded ComfyUI handoff, and reviewable workflow binding while external provider calls remain disabled by default. |
