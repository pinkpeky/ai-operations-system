...
| `DIGITAL_HUMAN_PROVIDER` | `mock` | Digital human provider selector. Current Phase 67D behavior supports local delivery manifests, guarded ComfyUI handoff, reviewable workflow binding, and operator-recorded workflow readiness while external provider calls remain disabled by default. |
