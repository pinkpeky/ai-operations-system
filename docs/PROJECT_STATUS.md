...
`codex/phase-61w-commercial-comfyui-adapter-dispatches` is the active next branch. It adds Phase 61W Commercial Operation ComfyUI Adapter Dispatches: `commercial_operation_comfyui_adapter_dispatches`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches`, Admin Dashboard ComfyUI adapter dispatch controls, probed-connection-to-adapter-dispatch review lifecycle, metadata-only prompt/workflow/queue/dispatch payload planning, guardrails, retry/recovery metadata, and operator-visible dispatch state for future guarded adapters. It still does not upload or ingest knowledge files, store secret values, auto-approve assets, auto-publish, execute external actions, run Browser Worker actions, run OpenClaw actions, call ComfyUI endpoints, read ComfyUI queues, upload files to ComfyUI, submit ComfyUI jobs, generate media, control real accounts, ingest platform analytics, claim ROI attribution, or bypass approval.
