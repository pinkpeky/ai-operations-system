...
`codex/phase-61s-commercial-comfyui-adapter-configs` is the active next branch. It adds Phase 61S Commercial Operation ComfyUI Adapter Configs: `commercial_operation_comfyui_adapter_configs`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-configs`, Admin Dashboard ComfyUI adapter config controls, endpoint/queue/workflow allowlist/model inventory/runtime-limit/secret-reference validation, and optional preflight config selection for future guarded handoffs. It still does not upload or ingest knowledge files, store secret values, auto-approve assets, auto-publish, execute external actions, run Browser Worker actions, run OpenClaw actions, call ComfyUI endpoints, submit ComfyUI jobs, generate media, control real accounts, ingest platform analytics, claim ROI attribution, or bypass approval.
