...
`codex/phase-61m-commercial-evidence-snapshots` is the active next branch. It adds Phase 61M Commercial Operation Evidence Snapshots: `commercial_operation_evidence_snapshots`, `/api/v1/commercial-operations/{operation_id}/evidence-snapshots`, an Admin Dashboard Evidence snapshots panel, approved evidence snapshot IDs on execution requests/runs, and operator checklists for human review before future execution. It still does not run live RAG retrieval, ingest knowledge files, auto-publish, execute external actions, run Browser Worker actions, run OpenClaw actions, run ComfyUI jobs, control real accounts, ingest platform analytics, claim ROI attribution, or bypass approval.
