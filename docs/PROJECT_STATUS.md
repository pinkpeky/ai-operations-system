...
`codex/phase-64d-frontend-operability-optimization` is the active next branch after draft PR #107 / Phase 64C Commercial Agent/Skill Orchestration. It adds Phase 64D Server/Client Frontend Operability Optimization: `admin_dashboard` now has a server maintenance cockpit for connection, customer-machine visibility, selected operation, and Agent/Skill next-action status, while `worker_console` and `worker_console_desktop` make the commercial operations first screen simpler by showing common actions first and folding advanced execution/recovery controls. This makes the closed loop easier for server maintainers and customer-machine operators to operate before future real adapter work. It does not execute live OpenClaw, run live Playwright publishing, publish to social media, control real accounts, call ComfyUI, ingest platform analytics, enable runtime switches, write environment variables, restart services, mutate runtime configuration, resolve secrets, bypass captcha, use proxy pools, bypass fingerprints, bypass approval, or rebuild client packages.
