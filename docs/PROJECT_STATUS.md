...
`codex/phase-62k-customer-console-codex-ux` is the active next branch and PR #77 draft. It adds Phase 62K Customer Console Codex-like UX Simplification after PR #76 / Phase 62J ComfyUI Runtime Guarded Probe Execution Audit: `worker_console` and `worker_console_desktop` now prioritize a concise customer-machine command surface with a left status rail, central next-step guidance, first-screen conversation input, neutral operational styling, and advanced maintenance/diagnostics collapsed behind a disclosure. This phase preserves local runtime/heartbeat controls, Chinese/English language switching, approvals, playbooks, outputs, tasks, logs, and server-vs-customer-machine boundary warnings. It does not import ComfyUI adapters, call ComfyUI, submit prompts, read or submit queues, upload files, generate media, enable runtime switches, write environment variables, restart services, mutate runtime configuration, resolve secrets, publish, run OpenClaw, control accounts, or bypass approval.
...
Phase 62I Workstation/Customer Client Frontend UX Alignment from `codex/phase-62i-workstation-client-ux` remains the immediate customer-console UX predecessor, and Phase 62J remains the immediate runtime predecessor for this slice: the server-side ComfyUI guarded probe audit is approval-gated, while Phase 62K keeps the customer-machine UI focused on simple operation rather than exposing dense maintenance internals by default.

Phase 62J still depends on Phase 62H ComfyUI Runtime Post-Manual Readiness Checks for the approved guarded read-only probe chain; Phase 62K does not loosen that runtime boundary.
