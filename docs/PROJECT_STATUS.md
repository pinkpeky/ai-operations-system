...
`codex/phase-61r-commercial-comfyui-preflight` is the active next branch. It adds Phase 61R Commercial Operation ComfyUI Preflights: `commercial_operation_comfyui_preflights`, `/api/v1/commercial-operations/{operation_id}/comfyui-preflights`, Admin Dashboard ComfyUI preflight controls, endpoint/queue/model/workflow readiness checks, and guarded adapter config normalization for approved/prepared handoffs. It still does not upload or ingest knowledge files, auto-approve assets, auto-publish, execute external actions, run Browser Worker actions, run OpenClaw actions, call ComfyUI endpoints, submit ComfyUI jobs, generate media, control real accounts, ingest platform analytics, claim ROI attribution, or bypass approval.
