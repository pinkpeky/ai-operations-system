...
`codex/phase-61q-commercial-comfyui-handoffs` is the active next branch. It adds Phase 61Q Commercial Operation ComfyUI Handoffs: `commercial_operation_comfyui_handoffs`, `/api/v1/commercial-operations/{operation_id}/comfyui-handoffs`, Admin Dashboard ComfyUI handoff controls, prompt/workflow payloads, readiness checks, and lifecycle decisions for approved/prepared asset requests. It still does not upload or ingest knowledge files, auto-approve assets, auto-publish, execute external actions, run Browser Worker actions, run OpenClaw actions, submit ComfyUI jobs, generate media, control real accounts, ingest platform analytics, claim ROI attribution, or bypass approval.
