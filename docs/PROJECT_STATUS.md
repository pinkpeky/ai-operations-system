...
`codex/phase-62a-comfyui-runtime-adapter-contract` is the active next branch. It adds Phase 62A ComfyUI Runtime Adapter Contract: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `ComfyUIRuntimeService`, explicit `COMFYUI_RUNTIME_*` settings, Docker environment exposure, docs/runtime verification, and a dedicated Admin Dashboard ComfyUI tab contract panel for provider, switch, base URL, allowlist, and guardrail visibility. It still does not upload or ingest knowledge files, store or resolve secret values, enable runtime switches, mutate runtime configuration, auto-approve assets, auto-publish, execute external actions, run Browser Worker actions, run OpenClaw actions, import or call ComfyUI adapters, call ComfyUI endpoints, read ComfyUI queues, upload files to ComfyUI, submit ComfyUI jobs, generate media, control real accounts, ingest platform analytics, claim ROI attribution, or bypass approval.
