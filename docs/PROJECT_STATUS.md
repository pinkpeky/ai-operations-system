...
`codex/phase-61z-commercial-comfyui-runtime-activations` is the active next branch. It adds Phase 61Z Commercial Operation ComfyUI Runtime Activations: `commercial_operation_comfyui_runtime_activations`, `/api/v1/commercial-operations/{operation_id}/comfyui-runtime-activations`, a dedicated Admin Dashboard ComfyUI tab with runtime activation controls, validated-runtime-dry-run-to-runtime-activation review lifecycle, metadata-only activation request records, switch audit records, runtime guardrails, validation checks, rollback guidance, and operator-visible runtime activation state for future guarded adapters. It still does not upload or ingest knowledge files, store or resolve secret values, enable runtime switches, auto-approve assets, auto-publish, execute external actions, run Browser Worker actions, run OpenClaw actions, import or call ComfyUI adapters, call ComfyUI endpoints, read ComfyUI queues, upload files to ComfyUI, submit ComfyUI jobs, generate media, control real accounts, ingest platform analytics, claim ROI attribution, or bypass approval.
