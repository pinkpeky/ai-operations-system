...
`codex/phase-61u-commercial-comfyui-execution-plans` is the active next branch. It adds Phase 61U Commercial Operation ComfyUI Execution Plans: `commercial_operation_comfyui_execution_plans`, `/api/v1/commercial-operations/{operation_id}/comfyui-execution-plans`, Admin Dashboard ComfyUI execution plan controls, approved/queued-job-request-to-execution-plan review lifecycle, queue simulation payload normalization, operator checklist, and rollback guidance for future guarded queue payloads. It still does not upload or ingest knowledge files, store secret values, auto-approve assets, auto-publish, execute external actions, run Browser Worker actions, run OpenClaw actions, call ComfyUI endpoints, upload files to ComfyUI, submit ComfyUI jobs, generate media, control real accounts, ingest platform analytics, claim ROI attribution, or bypass approval.
