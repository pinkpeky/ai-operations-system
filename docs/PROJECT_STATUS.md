...
`codex/phase-61h-commercial-operation-execution-requests` is the active next branch. It adds Phase 61H Commercial Operation Execution Requests: `commercial_operation_execution_requests`, `/api/v1/commercial-operations/{operation_id}/execution-requests`, an Admin Dashboard Execution requests panel, and explicit create/update/ready/approve/reject/prepare/fail/cancel/archive records for metadata-only future runtime handoff requests from packaged deliverables. It still does not publish, execute external actions, run Browser Worker actions, run OpenClaw actions, run ComfyUI jobs, control real accounts, or bypass approval.
