...
Current effective phase: Phase 62C ComfyUI Runtime Diagnostics. Phase 56 was reverted and is not active. The old reverted Phase 56 branch is not active, not part of the accepted baseline, and should not be reused.

The current fresh branch is `codex/phase-62c-comfyui-runtime-diagnostics`. It stacks after draft PR #68 and adds no-network ComfyUI runtime diagnostics: `/api/v1/comfyui-runtime/diagnostics`, `readiness_status`, `blocking_reasons`, `recommended_actions`, `read_only_probe_ready`, and per-gate checks for provider, runtime switch, network gate, base URL scheme, host allowlist, read-only probe gate, health path allowlist, and execution boundary. It does not import or call a ComfyUI adapter, call execution endpoints, read ComfyUI queues, submit prompts or queues, upload files, generate images/videos, publish, control accounts, enable runtime server switches, mutate runtime configuration, execute OpenClaw, call Browser Worker actions, store or resolve secret values, ingest platform analytics, or bypass approval.
