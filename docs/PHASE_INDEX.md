...
Current effective phase: Phase 62E ComfyUI Runtime Maintenance Runbook. Phase 56 was reverted and is not active. The old reverted Phase 56 branch is not active, not part of the accepted baseline, and should not be reused.

The current fresh branch is `codex/phase-62e-comfyui-maintenance-console`. It stacks after draft PR #70 and adds a no-network ComfyUI runtime maintenance runbook: `GET /api/v1/comfyui-runtime/maintenance-runbook`, ordered operator steps, `next_operator_action`, `recovery_actions`, `configuration_summary`, `snapshot_recommended`, disabled-action visibility, and Admin Dashboard ComfyUI page runbook controls. The runbook API reuses Phase 62C diagnostics and does not import or call a ComfyUI adapter, call execution endpoints, read ComfyUI queues, submit prompts or queues, upload files, generate images/videos, publish, control accounts, enable runtime server switches, mutate runtime configuration, execute OpenClaw, call Browser Worker actions, store or resolve secret values, ingest platform analytics, or bypass approval.
