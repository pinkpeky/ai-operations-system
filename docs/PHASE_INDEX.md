...
Current effective phase: Phase 61Y Commercial Operation ComfyUI Runtime Dry-Runs. Phase 56 was reverted and is not active. The old reverted Phase 56 branch is not active, not part of the accepted baseline, and should not be reused.

The current fresh branch is `codex/phase-61y-commercial-comfyui-runtime-dry-runs`. It stacks after draft PR #64 and adds `/api/v1/commercial-operations/{operation_id}/comfyui-runtime-dry-runs` plus `/api/v1/commercial-operations/{operation_id}/comfyui-runtime-gates/{runtime_gate_id}/runtime-dry-runs`, which lets operators and server maintainers turn armed ComfyUI runtime gates into reviewable metadata-only runtime dry-run records with adapter contract metadata, request fixtures, expected response contracts, explicit server switch policy, validation checks, operator checklists, rollback guidance, lifecycle decisions, and plan-step runtime-dry-run state. It still performs local metadata evaluation only; it does not import or call a ComfyUI adapter, call ComfyUI, read ComfyUI queues, submit prompts or queues, generate images/videos, upload files, publish, control accounts, enable runtime server switches, execute OpenClaw, call Browser Worker actions, store or resolve secret values, ingest platform analytics, or bypass approval.
