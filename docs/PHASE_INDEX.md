...
Current effective phase: Phase 62D ComfyUI Runtime Diagnostic Snapshots. Phase 56 was reverted and is not active. The old reverted Phase 56 branch is not active, not part of the accepted baseline, and should not be reused.

The current fresh branch is `codex/phase-62d-comfyui-runtime-diagnostic-snapshots`. It stacks after draft PR #69 and adds persisted no-network ComfyUI runtime diagnostic snapshots: `comfyui_runtime_diagnostic_snapshots`, `POST /api/v1/comfyui-runtime/diagnostic-snapshots`, `GET /api/v1/comfyui-runtime/diagnostic-snapshots`, operator notes, snapshot metadata, recent snapshot visibility, and Admin Dashboard ComfyUI page save controls. The snapshot API reuses Phase 62C diagnostics and does not import or call a ComfyUI adapter, call execution endpoints, read ComfyUI queues, submit prompts or queues, upload files, generate images/videos, publish, control accounts, enable runtime server switches, mutate runtime configuration, execute OpenClaw, call Browser Worker actions, store or resolve secret values, ingest platform analytics, or bypass approval.
