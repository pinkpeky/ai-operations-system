...
Current effective phase: Phase 61U Commercial Operation ComfyUI Execution Plans. Phase 56 was reverted and is not active. The old reverted Phase 56 branch is not active, not part of the accepted baseline, and should not be reused.

The current fresh branch is `codex/phase-61u-commercial-comfyui-execution-plans`. It stacks after draft PR #60 and adds `/api/v1/commercial-operations/{operation_id}/comfyui-execution-plans`, which lets operators turn approved or queued ComfyUI job requests into reviewable metadata-only queue simulation plans with execution steps, local checks, rollback guidance, lifecycle decisions, and plan-step execution-plan state. It still performs local metadata evaluation only; it does not call ComfyUI, submit queues, generate images/videos, upload files, publish, control accounts, execute OpenClaw, call Browser Worker actions, store secret values, ingest platform analytics, or bypass approval.
