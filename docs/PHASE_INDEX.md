...
Current effective phase: Phase 61X Commercial Operation ComfyUI Runtime Gates. Phase 56 was reverted and is not active. The old reverted Phase 56 branch is not active, not part of the accepted baseline, and should not be reused.

The current fresh branch is `codex/phase-61x-commercial-comfyui-runtime-gates`. It stacks after draft PR #63 and adds `/api/v1/commercial-operations/{operation_id}/comfyui-runtime-gates` plus `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/runtime-gates`, which lets operators and server maintainers turn recorded ComfyUI adapter dispatches into reviewable metadata-only runtime gate records with runtime switch metadata, network policy, queue policy, secret-reference policy, approval policy, validation checks, operator checklists, rollback guidance, lifecycle decisions, and plan-step runtime-gate state. It still performs local metadata evaluation only; it does not call ComfyUI, read ComfyUI queues, submit prompts or queues, generate images/videos, upload files, publish, control accounts, execute OpenClaw, call Browser Worker actions, store or resolve secret values, ingest platform analytics, or bypass approval.
