...
Current effective phase: Phase 61V Commercial Operation ComfyUI Connection Probes. Phase 56 was reverted and is not active. The old reverted Phase 56 branch is not active, not part of the accepted baseline, and should not be reused.

The current fresh branch is `codex/phase-61v-commercial-comfyui-connection-probes`. It stacks after draft PR #61 and adds `/api/v1/commercial-operations/{operation_id}/comfyui-connection-probes` plus `/api/v1/commercial-operations/{operation_id}/comfyui-execution-plans/{execution_plan_id}/connection-probes`, which lets operators turn approved or simulated ComfyUI execution plans into reviewable metadata-only connection probe records with documented health/queue routes, readiness checks, sanitized snapshots, lifecycle decisions, and plan-step connection-probe state. It still performs local metadata evaluation only; it does not call ComfyUI, read ComfyUI queues, submit queues, generate images/videos, upload files, publish, control accounts, execute OpenClaw, call Browser Worker actions, store secret values, ingest platform analytics, or bypass approval.
