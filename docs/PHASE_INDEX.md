...
Current effective phase: Phase 62B ComfyUI Guarded Read-Only Probe. Phase 56 was reverted and is not active. The old reverted Phase 56 branch is not active, not part of the accepted baseline, and should not be reused.

The current fresh branch is `codex/phase-62b-comfyui-guarded-readonly-probe`. It stacks after draft PR #67 and adds the guarded read-only ComfyUI health probe: default no-network behavior, `COMFYUI_RUNTIME_READ_ONLY_PROBE_ENABLED`, `COMFYUI_RUNTIME_HEALTH_PATH`, `COMFYUI_RUNTIME_ALLOWED_HEALTH_PATHS`, host/path allowlist enforcement, `GET /system_stats` only when every explicit gate is enabled, Docker environment exposure, docs/runtime verification, and a dedicated Admin Dashboard ComfyUI tab probe panel for operators and server maintainers. It does not import or call a ComfyUI adapter, call execution endpoints, read ComfyUI queues, submit prompts or queues, upload files, generate images/videos, publish, control accounts, enable runtime server switches, mutate runtime configuration, execute OpenClaw, call Browser Worker actions, store or resolve secret values, ingest platform analytics, or bypass approval.
