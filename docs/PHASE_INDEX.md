...
Current effective phase: Phase 62F ComfyUI Runtime Configuration Change Requests. Phase 56 was reverted and is not active. The old reverted Phase 56 branch is not active, not part of the accepted baseline, and should not be reused.

The current fresh branch is `codex/phase-62f-comfyui-config-change-requests`. It stacks after draft PR #71 and adds no-network, metadata-only ComfyUI runtime configuration change requests: `comfyui_runtime_config_change_requests`, `POST /api/v1/comfyui-runtime/config-change-requests`, `GET /api/v1/comfyui-runtime/config-change-requests`, ready/approve/reject/cancel/archive review actions, `change_status`, `requested_changes`, and `config_mutation_performed=false` audit fields derived from the Phase 62E maintenance runbook. The request APIs do not import or call a ComfyUI adapter, call execution endpoints, read ComfyUI queues, submit prompts or queues, upload files, generate images/videos, publish, control accounts, enable runtime server switches, mutate runtime configuration, execute OpenClaw, call Browser Worker actions, store or resolve secret values, ingest platform analytics, or bypass approval.
