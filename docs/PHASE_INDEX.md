...
Current effective phase: Phase 61T Commercial Operation ComfyUI Job Requests. Phase 56 was reverted and is not active. The old reverted Phase 56 branch is not active, not part of the accepted baseline, and should not be reused.

The current fresh branch is `codex/phase-61t-commercial-comfyui-job-requests`. It stacks after draft PR #59 and adds `/api/v1/commercial-operations/{operation_id}/comfyui-job-requests`, which lets operators turn checked ComfyUI preflights into reviewable future job request payloads with safety checks, output expectations, lifecycle decisions, and recovery guidance. It still performs local metadata evaluation only; it does not call ComfyUI, submit queues, generate images/videos, upload files, publish, control accounts, execute OpenClaw, call Browser Worker actions, store secret values, ingest platform analytics, or bypass approval.
