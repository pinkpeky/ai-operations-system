...
Current effective phase: Phase 62G ComfyUI Runtime Manual Apply Evidence. Phase 56 was reverted and is not active. The old reverted Phase 56 branch is not active, not part of the accepted baseline, and should not be reused.

The current fresh branch is `codex/phase-62g-comfyui-manual-apply-evidence`. It stacks after draft PR #72 and adds no-network, metadata-only ComfyUI runtime manual apply evidence: `comfyui_runtime_manual_apply_evidence`, `ComfyUIRuntimeManualApplyEvidence`, `POST /api/v1/comfyui-runtime/config-change-requests/{request_id}/manual-apply-evidence`, `GET /api/v1/comfyui-runtime/manual-apply-evidence`, ready/verify/reject/fail/archive review actions, `manual_config_applied`, `service_restart_reported`, and `api_config_mutation_performed=false` audit fields derived from an approved Phase 62F request. The evidence APIs do not import or call a ComfyUI adapter, call execution endpoints, read ComfyUI queues, submit prompts or queues, upload files, generate images/videos, publish, control accounts, enable runtime server switches, write environment variables, restart services, mutate runtime configuration, execute OpenClaw, call Browser Worker actions, store or resolve secret values, ingest platform analytics, or bypass approval.
