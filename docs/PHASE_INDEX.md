...
Current effective phase: Phase 62A ComfyUI Runtime Adapter Contract. Phase 56 was reverted and is not active. The old reverted Phase 56 branch is not active, not part of the accepted baseline, and should not be reused.

The current fresh branch is `codex/phase-62a-comfyui-runtime-adapter-contract`. It stacks after draft PR #66 and adds the disabled-by-default ComfyUI runtime adapter contract: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `ComfyUIRuntimeService`, explicit `COMFYUI_RUNTIME_*` configuration, Docker environment exposure, docs/runtime verification, and a dedicated Admin Dashboard ComfyUI tab contract panel for operators and server maintainers. It still performs contract evaluation only; it does not import or call a ComfyUI adapter, call ComfyUI endpoints, read ComfyUI queues, submit prompts or queues, upload files, generate images/videos, publish, control accounts, enable runtime server switches, mutate runtime configuration, execute OpenClaw, call Browser Worker actions, store or resolve secret values, ingest platform analytics, or bypass approval.
