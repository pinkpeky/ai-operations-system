...
Current effective phase: Phase 61W Commercial Operation ComfyUI Adapter Dispatches. Phase 56 was reverted and is not active. The old reverted Phase 56 branch is not active, not part of the accepted baseline, and should not be reused.

The current fresh branch is `codex/phase-61w-commercial-comfyui-adapter-dispatches`. It stacks after draft PR #62 and adds `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches` plus `/api/v1/commercial-operations/{operation_id}/comfyui-connection-probes/{connection_probe_id}/adapter-dispatches`, which lets operators turn recorded ComfyUI connection probes into reviewable metadata-only adapter dispatch handoffs with prompt/workflow/queue payloads, guardrails, operator checklists, retry policy, recovery plan, lifecycle decisions, and plan-step dispatch state. It still performs local metadata evaluation only; it does not call ComfyUI, read ComfyUI queues, submit queues, generate images/videos, upload files, publish, control accounts, execute OpenClaw, call Browser Worker actions, store secret values, ingest platform analytics, or bypass approval.
