...
```text
codex/phase-62i-workstation-client-ux
```
...
- A Commercial Ops ComfyUI execution plan that turns approved or queued job requests into metadata-only queue simulation records with execution steps, local checks, operator checklist, and rollback guidance without calling ComfyUI, uploading files, or submitting queues.
- A Commercial Ops ComfyUI connection probe that turns approved or simulated execution plans into metadata-only health and queue snapshot plans with route/readiness checks and sanitized probe payloads without calling ComfyUI, reading queues, uploading files, or submitting queues.
- A ComfyUI Runtime Adapter Contract page that lets a server maintainer inspect provider, enabled switch, base URL, timeout, host/path allowlists, required configuration, disabled actions, guarded read-only probe status, `readiness_status`, `blocking_reasons`, `recommended_actions`, `next_operator_action`, `recovery_actions`, the maintenance runbook table, metadata-only configuration change requests, manual apply evidence, and post-manual readiness checks. Diagnostics, runbook, snapshots, config change request creation, manual apply evidence creation, and post-manual readiness checks make no network request; health only attempts `GET /system_stats` when every explicit Phase 62B gate is enabled.
...
- Phase 61T Commercial Operation ComfyUI Job Requests adds metadata-only future queue payload records from checked preflights; it does not call ComfyUI endpoints, upload files, submit queues, generate media, publish, or execute external actions.
- Phase 61U Commercial Operation ComfyUI Execution Plans adds metadata-only queue simulation records from approved or queued job requests; it does not call ComfyUI endpoints, upload files, submit queues, generate media, publish, or execute external actions.
- Phase 61V Commercial Operation ComfyUI Connection Probes adds metadata-only health and queue snapshot plans from approved or simulated execution plans; it does not call ComfyUI endpoints, read queues, upload files, submit queues, generate media, publish, or execute external actions.
- Phase 61W Commercial Operation ComfyUI Adapter Dispatches adds metadata-only guarded dispatch handoffs from recorded connection probes; it does not call ComfyUI endpoints, submit prompts, read queues, upload files, submit queues, generate media, publish, or execute external actions.
- Phase 61X Commercial Operation ComfyUI Runtime Gates adds metadata-only runtime switch, network policy, queue policy, secret-reference policy, approval policy, validation, and rollback records from recorded adapter dispatches; it does not call ComfyUI endpoints, submit prompts, read queues, upload files, submit queues, resolve secrets, generate media, publish, or execute external actions.
- Phase 61Y Commercial Operation ComfyUI Runtime Dry-Runs adds `commercial_operation_comfyui_runtime_dry_runs` and `/api/v1/commercial-operations/{operation_id}/comfyui-runtime-dry-runs` for metadata-only adapter contract, dry-run request fixture, expected response contract, explicit server switch policy, validation, and rollback records from armed runtime gates; it does not import or call ComfyUI adapters, call ComfyUI endpoints, enable runtime switches, submit prompts, read queues, upload files, submit queues, resolve secrets, generate media, publish, or execute external actions.
- Phase 61Z Commercial Operation ComfyUI Runtime Activations adds `commercial_operation_comfyui_runtime_activations` and `/api/v1/commercial-operations/{operation_id}/comfyui-runtime-activations` for metadata-only activation request, switch audit, runtime guardrail, validation, and rollback records from validated runtime dry-runs; the Admin Dashboard exposes these controls in the dedicated `?page=comfyui-operations` tab while Commercial Ops remains focused on the business workflow; it does not import or call ComfyUI adapters, call ComfyUI endpoints, enable runtime switches, submit prompts, read queues, upload files, submit queues, resolve secrets, generate media, publish, or execute external actions.
- Phase 62A ComfyUI Runtime Adapter Contract adds `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `ComfyUIRuntimeService`, explicit `COMFYUI_RUNTIME_*` settings, Docker environment exposure, and a dedicated ComfyUI tab runtime adapter contract panel; it does not import or call ComfyUI adapters, call ComfyUI endpoints, enable runtime switches, mutate runtime configuration, submit prompts, read queues, upload files, submit queues, resolve secrets, generate media, publish, or execute external actions.
- Phase 62B ComfyUI Guarded Read-Only Probe adds `COMFYUI_RUNTIME_READ_ONLY_PROBE_ENABLED`, `COMFYUI_RUNTIME_HEALTH_PATH`, `COMFYUI_RUNTIME_ALLOWED_HEALTH_PATHS`, guarded `GET /system_stats` only after provider/enabled/network/host/path gates are true, and ComfyUI tab probe status fields; it does not import or call adapters, submit prompts, read or submit queues, upload files, generate media, enable runtime switches, mutate runtime configuration, resolve secrets, publish, or execute external actions.
- Phase 62C ComfyUI Runtime Diagnostics adds `/api/v1/comfyui-runtime/diagnostics`, `readiness_status`, `blocking_reasons`, `recommended_actions`, `read_only_probe_ready`, and per-gate checks for provider/switch/network/host/path readiness; diagnostics never calls ComfyUI, imports adapters, submits prompts, reads or submits queues, uploads files, generates media, enables runtime switches, mutates runtime configuration, resolves secrets, publishes, or executes external actions.
- Phase 62D ComfyUI Runtime Diagnostic Snapshots adds `comfyui_runtime_diagnostic_snapshots`, `/api/v1/comfyui-runtime/diagnostic-snapshots`, persisted operator notes/metadata, recent snapshot fields, and a ComfyUI tab save action; snapshots reuse no-network diagnostics and never call ComfyUI, import adapters, submit prompts, read or submit queues, upload files, generate media, enable runtime switches, mutate runtime configuration, resolve secrets, publish, or execute external actions.
- Phase 62E ComfyUI Runtime Maintenance Runbook adds `/api/v1/comfyui-runtime/maintenance-runbook`, ordered maintainer steps, `next_operator_action`, `recovery_actions`, `configuration_summary`, `snapshot_recommended`, disabled-action visibility, and a ComfyUI tab runbook table; the runbook reuses no-network diagnostics and never calls ComfyUI, import adapters, submit prompts, read or submit queues, upload files, generate media, enable runtime switches, mutate runtime configuration, resolve secrets, publish, or execute external actions.
- Phase 62F ComfyUI Runtime Configuration Change Requests adds `comfyui_runtime_config_change_requests`, `ComfyUIRuntimeConfigChangeRequest`, `/api/v1/comfyui-runtime/config-change-requests`, ready/approve/reject/cancel/archive review actions, `change_status`, `requested_changes`, and `config_mutation_performed=false` records derived from the maintenance runbook; the requests never call ComfyUI, import adapters, submit prompts, read or submit queues, upload files, generate media, enable runtime switches, mutate runtime configuration, resolve secrets, publish, or execute external actions.
- Phase 62G ComfyUI Runtime Manual Apply Evidence adds `comfyui_runtime_manual_apply_evidence`, `ComfyUIRuntimeManualApplyEvidence`, `/api/v1/comfyui-runtime/manual-apply-evidence`, `/api/v1/comfyui-runtime/config-change-requests/{request_id}/manual-apply-evidence`, ready/verify/reject/fail/archive review actions, restart evidence, rollback notes, verification notes, `manual_config_applied`, `service_restart_reported`, and `api_config_mutation_performed=false` records derived from approved config change requests; the evidence APIs never call ComfyUI, import adapters, submit prompts, read or submit queues, upload files, generate media, enable runtime switches, write environment variables, restart services, mutate runtime configuration, resolve secrets, publish, or execute external actions.
- Phase 62H ComfyUI Runtime Post-Manual Readiness Checks adds `comfyui_runtime_post_manual_readiness_checks`, `ComfyUIRuntimePostManualReadinessCheck`, `/api/v1/comfyui-runtime/post-manual-readiness-checks`, `/api/v1/comfyui-runtime/manual-apply-evidence/{evidence_id}/post-manual-readiness-checks`, ready/approve/reject/fail/archive review actions, readiness before/after/current comparison, `comparison_status`, `guarded_probe_ready`, `health_probe_executed=false`, and `api_config_mutation_performed=false` records derived from verified manual apply evidence; the readiness APIs never call `/system_stats`, ComfyUI, import adapters, submit prompts, read or submit queues, upload files, generate media, enable runtime switches, write environment variables, restart services, mutate runtime configuration, resolve secrets, publish, or execute external actions.
- Phase 62I Workstation/Customer Client Frontend UX Alignment adds a simplified operator home to `worker_console` and `worker_console_desktop`, with local connection/runtime/heartbeat/recovery status cards, runtime and heartbeat controls, shortcuts for conversation/playbook/approval/output/task/log workflows, setup/help panels, recovery guidance, Chinese/English language switching, and server/client boundary warnings. It does not add live ComfyUI calls, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha bypass, proxy pools, fingerprint bypass, secret resolution, or approval bypass.
- Phase 62J ComfyUI Runtime Guarded Probe Execution Audit adds `comfyui_runtime_guarded_probe_executions`, `ComfyUIRuntimeGuardedProbeExecution`, `/api/v1/comfyui-runtime/guarded-probe-executions`, `/api/v1/comfyui-runtime/post-manual-readiness-checks/{check_id}/guarded-probe-executions`, review actions, and an explicit execute action. Create/list/review stay no-network; the Admin Dashboard ComfyUI page auto-loads diagnostics instead of `/health`, so refresh does not run a probe; execute rechecks diagnostics and can call only the existing guarded `GET /system_stats` health path after approval, while prompt submission, queue reads/submissions, uploads, media generation, switch mutation, config mutation, secret resolution, publishing, OpenClaw, Browser Worker actions, and account control remain disabled.
- Phase 65A ComfyUI Real Adapter adds guarded `/api/v1/comfyui-runtime/prompt-jobs`, `/api/v1/comfyui-runtime/prompt-jobs/{prompt_id}/history`, and `/api/v1/comfyui-runtime/queue` paths plus `COMFYUI_RUNTIME_PROMPT_SUBMISSION_ENABLED` and `COMFYUI_RUNTIME_ALLOWED_EXECUTION_PATHS`. The Admin Dashboard ComfyUI tab shows queue status and a smoke prompt control. Calls remain disabled by default and do not upload files, resolve secrets, publish, control accounts, mutate runtime config, restart services, bypass approval, or run OpenClaw/Playwright publishing.
- Phase 61A adds the adjacent Commercial Ops project center. Phase 61B adds manual evidence and handoff links from Commercial Ops back to conversations, artifacts, tasks, workflows, RAG documents, approvals, and external materials. Phase 61C adds `commercial_operation_approvals` and `/api/v1/commercial-operations/{operation_id}/approvals` for individual Commercial Ops plan-step approval gates. Phase 61D adds `commercial_operation_dry_runs` and `/api/v1/commercial-operations/{operation_id}/dry-runs` for approved, metadata-only execution preparation records. Phase 61E adds `commercial_operation_content_drafts` and `/api/v1/commercial-operations/{operation_id}/content-drafts` for reviewable channel drafts. Phase 61F adds `commercial_operation_asset_requests` and `/api/v1/commercial-operations/{operation_id}/asset-requests` for first-class asset handoff preparation records. Phase 61G adds `commercial_operation_deliverables` and `/api/v1/commercial-operations/{operation_id}/deliverables` for approved commercial handoff packages linked to Output Library artifacts. Phase 61H adds `commercial_operation_execution_requests` and `/api/v1/commercial-operations/{operation_id}/execution-requests` for metadata-only future runtime handoff records from packaged deliverables. Phase 61I adds `commercial_operation_execution_runs` and `/api/v1/commercial-operations/{operation_id}/execution-runs` for metadata-only run lifecycle, retry, result, and recovery records from prepared execution requests. Phase 61J adds `commercial_operation_results` and `/api/v1/commercial-operations/{operation_id}/results` for operator-observed commercial result reports from terminal execution runs. Phase 61K adds `commercial_operation_monitoring_observations` and `/api/v1/commercial-operations/{operation_id}/monitoring-observations` for operator-observed monitoring snapshots from approved commercial results. Phase 61L adds `commercial_operation_optimization_decisions` and `/api/v1/commercial-operations/{operation_id}/optimization-decisions` for operator-decided optimization actions from approved monitoring observations. Phase 61M adds `commercial_operation_evidence_snapshots` and `/api/v1/commercial-operations/{operation_id}/evidence-snapshots` for approved evidence packages, execution request/run evidence IDs, and operator checklists. Phase 61N adds `/api/v1/commercial-operations/{operation_id}/evidence-snapshots/generate-rag` for draft evidence generated from existing RAG search. Phase 61O adds `/api/v1/commercial-operations/{operation_id}/content-drafts/generate-rag` for draft content generated from existing RAG search. Phase 61P adds `/api/v1/commercial-operations/{operation_id}/asset-requests/generate-rag` for draft asset briefs generated from existing RAG search. Phase 61Q adds `commercial_operation_comfyui_handoffs` and `/api/v1/commercial-operations/{operation_id}/comfyui-handoffs` for metadata-only ComfyUI handoff records. Phase 61R adds `commercial_operation_comfyui_preflights` and `/api/v1/commercial-operations/{operation_id}/comfyui-preflights` for metadata-only ComfyUI readiness records. Phase 61S adds `commercial_operation_comfyui_adapter_configs` and `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-configs` for metadata-only ComfyUI adapter config records. Phase 61T adds `commercial_operation_comfyui_job_requests` and `/api/v1/commercial-operations/{operation_id}/comfyui-job-requests` for metadata-only ComfyUI job request records. Phase 61U adds `commercial_operation_comfyui_execution_plans` and `/api/v1/commercial-operations/{operation_id}/comfyui-execution-plans` for metadata-only ComfyUI execution plan records. Phase 61V adds `commercial_operation_comfyui_connection_probes` and `/api/v1/commercial-operations/{operation_id}/comfyui-connection-probes` for metadata-only ComfyUI connection probe records. Phase 61W adds `commercial_operation_comfyui_adapter_dispatches` and `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches` for metadata-only ComfyUI adapter dispatch records. Phase 61X adds `commercial_operation_comfyui_runtime_gates` and `/api/v1/commercial-operations/{operation_id}/comfyui-runtime-gates` for metadata-only ComfyUI runtime gate records. Run Cockpit still remains the execution-monitoring surface; Commercial Ops is the concise goal-to-plan, handoff, approval-gate, content-draft, asset-request, deliverable, evidence-snapshot, execution-request, execution-run, result, monitoring observation, optimization decision, and dry-run entry point, while the dedicated ComfyUI tab owns ComfyUI handoffs, preflights, adapter configs, job requests, execution plans, connection probes, adapter dispatches, runtime gates, runtime dry-runs, and runtime activations. Neither surface publishes, executes external actions, ingests platform analytics, or claims ROI attribution.
