...
```text
codex/phase-62b-comfyui-guarded-readonly-probe
```
...
- A ComfyUI Runtime Adapter Contract page that lets a server maintainer inspect provider, enabled switch, base URL, timeout, host/path allowlists, required configuration, disabled actions, and guarded read-only probe status. It makes no network request by default and only attempts `GET /system_stats` when every explicit Phase 62B gate is enabled.
