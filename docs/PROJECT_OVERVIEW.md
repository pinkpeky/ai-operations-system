...
The current next branch is `codex/phase-62k-customer-console-codex-ux` for Phase 62K Customer Console Codex-like UX Simplification. It responds to customer-machine UI feedback by refactoring `worker_console` and `worker_console_desktop` so operators see a concise command surface first: left status rail, central next-step guidance, first-screen conversation input, neutral operational styling, and advanced maintenance/diagnostics collapsed behind a disclosure. Phase 62K does not import ComfyUI adapters, call ComfyUI, submit prompts, read or submit queues, upload files, generate media, enable runtime switches, write environment variables, restart services, mutate runtime configuration, resolve secrets, publish, run OpenClaw, control accounts, or bypass approval.

Phase 62K is stacked after Phase 62J ComfyUI Runtime Guarded Probe Execution Audit and Phase 62I Workstation/Customer Client Frontend UX Alignment. The immediate operator surfaces now include the Admin Dashboard ComfyUI page plus the simplified customer-machine `worker_console` and `worker_console_desktop` command surfaces. Server-side ComfyUI probe execution remains approval-gated; customer-machine UI work remains frontend-only.

Phase 62J still inherits the Phase 62H ComfyUI Runtime Post-Manual Readiness Checks boundary for guarded probe approval, including `manual_config_applied`, `api_config_mutation_performed`, `guarded_probe_ready`, `health_probe_executed`, `external_request_attempted`, and `probe_result_status` audit fields; Phase 62K only changes the customer-machine frontend layout.
