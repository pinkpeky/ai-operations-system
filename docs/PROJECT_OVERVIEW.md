...
The current next branch is `codex/phase-62n-client-goal-plan-preview` for Phase 62N Customer Console Goal Plan Preview. It builds on the customer-machine goal templates by adding a compact plan preview to `worker_console` and `worker_console_desktop`; selecting a template shows planned steps, approval boundary, and expected output before the operator clicks run. Phase 62N does not import ComfyUI adapters, call ComfyUI, submit prompts, read or submit queues, upload files, generate media, enable runtime switches, write environment variables, restart services, mutate runtime configuration, resolve secrets, publish, run OpenClaw, control accounts, or bypass approval.

Phase 62N is stacked after Phase 62M Customer Console Goal Templates on `codex/phase-62m-client-goal-templates`, Phase 62L Customer Console Task Workbench on `codex/phase-62l-client-task-workbench`, Phase 62K Customer Console Codex-like UX Simplification, and Phase 62J ComfyUI Runtime Guarded Probe Execution Audit. The immediate operator surfaces now include the Admin Dashboard ComfyUI page plus the simplified customer-machine `worker_console` and `worker_console_desktop` task workbench surfaces with selectable goal templates and plan previews. Server-side ComfyUI probe execution remains approval-gated; customer-machine UI work remains frontend-only.

Phase 62J still inherits the Phase 62H ComfyUI Runtime Post-Manual Readiness Checks boundary for guarded probe approval, including `manual_config_applied`, `api_config_mutation_performed`, `guarded_probe_ready`, `health_probe_executed`, `external_request_attempted`, and `probe_result_status` audit fields; Phase 62N only changes the customer-machine frontend goal-planning flow.
