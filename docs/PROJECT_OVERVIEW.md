...
The current next branch is `codex/phase-60-overview-persona-simplification`, scoped to a Chinese-first Overview role switch for workstation operators and server maintainers, role-specific entry cards, localized overview metrics, and concise snapshot labels without changing execution semantics.
