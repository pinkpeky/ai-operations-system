...
The current next branch is `codex/phase-62e-comfyui-maintenance-console`, scoped to Phase 62E ComfyUI Runtime Maintenance Runbook: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `/api/v1/comfyui-runtime/diagnostics`, `/api/v1/comfyui-runtime/maintenance-runbook`, `POST /api/v1/comfyui-runtime/diagnostic-snapshots`, `GET /api/v1/comfyui-runtime/diagnostic-snapshots`, `ComfyUIRuntimeService`, `readiness_status`, `blocking_reasons`, `recommended_actions`, `next_operator_action`, `recovery_actions`, `configuration_summary`, `snapshot_recommended`, and a dedicated Admin Dashboard ComfyUI tab runbook table. The diagnostics, runbook, and snapshot endpoints remain no-network. The only permitted live request is still the Phase 62B `GET /system_stats` health probe after every explicit provider/enabled/network/host/path/read-only gate is true, before any real ComfyUI adapter import, execution endpoint call, prompt submission, queue read, file upload, queue submission, generation, runtime switch enablement, config mutation, or secret resolution.
...
- ComfyUI Runtime Diagnostics, Maintenance Runbook, and Diagnostic Snapshots with `/api/v1/comfyui-runtime/diagnostics`, `/api/v1/comfyui-runtime/maintenance-runbook`, `/api/v1/comfyui-runtime/diagnostic-snapshots`, `comfyui_runtime_diagnostic_snapshots`, no-network readiness reporting, ordered operator actions, persisted operator notes/metadata, and Admin Dashboard runbook/snapshot visibility without adapter import/call, prompt submission, queue read/submission, upload, generation, runtime switch enablement, config mutation, or secret resolution.
