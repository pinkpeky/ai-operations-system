...
The current next branch is `codex/phase-62b-comfyui-guarded-readonly-probe`, scoped to Phase 62B ComfyUI Guarded Read-Only Probe: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `ComfyUIRuntimeService`, `COMFYUI_RUNTIME_READ_ONLY_PROBE_ENABLED`, `COMFYUI_RUNTIME_HEALTH_PATH`, `COMFYUI_RUNTIME_ALLOWED_HEALTH_PATHS`, Docker environment exposure, docs/runtime verification, and a dedicated Admin Dashboard ComfyUI tab probe panel. The default remains no-network. The only permitted live request is `GET /system_stats` after every explicit provider/enabled/network/host/path gate is true, before any real ComfyUI adapter import, execution endpoint call, prompt submission, queue read, file upload, queue submission, generation, runtime switch enablement, config mutation, or secret resolution.
