...
The current next branch is `codex/phase-62g-comfyui-manual-apply-evidence`, scoped to Phase 62G ComfyUI Runtime Manual Apply Evidence: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `/api/v1/comfyui-runtime/diagnostics`, `/api/v1/comfyui-runtime/maintenance-runbook`, `POST /api/v1/comfyui-runtime/config-change-requests`, `GET /api/v1/comfyui-runtime/config-change-requests`, ready/approve/reject/cancel/archive request actions, `POST /api/v1/comfyui-runtime/config-change-requests/{request_id}/manual-apply-evidence`, `GET /api/v1/comfyui-runtime/manual-apply-evidence`, ready/verify/reject/fail/archive evidence actions, `POST /api/v1/comfyui-runtime/diagnostic-snapshots`, `GET /api/v1/comfyui-runtime/diagnostic-snapshots`, `comfyui_runtime_manual_apply_evidence`, `ComfyUIRuntimeManualApplyEvidence`, `ComfyUIRuntimeService`, `readiness_status`, `blocking_reasons`, `recommended_actions`, `next_operator_action`, `recovery_actions`, `configuration_summary`, `snapshot_recommended`, `change_status`, `requested_changes`, `manual_config_applied=true`, `service_restart_reported`, `api_config_mutation_performed=false`, and a dedicated Admin Dashboard ComfyUI tab evidence table. The diagnostics, runbook, snapshot, config change request, and manual apply evidence endpoints remain no-network. The only permitted live request is still the Phase 62B `GET /system_stats` health probe after every explicit provider/enabled/network/host/path/read-only gate is true, before any real ComfyUI adapter import, execution endpoint call, prompt submission, queue read, file upload, queue submission, generation, runtime switch enablement, config mutation, environment write, service restart, or secret resolution.
...
- ComfyUI Runtime Diagnostics, Maintenance Runbook, Diagnostic Snapshots, and Configuration Change Requests with `/api/v1/comfyui-runtime/diagnostics`, `/api/v1/comfyui-runtime/maintenance-runbook`, `/api/v1/comfyui-runtime/diagnostic-snapshots`, `/api/v1/comfyui-runtime/config-change-requests`, `comfyui_runtime_diagnostic_snapshots`, `comfyui_runtime_config_change_requests`, no-network readiness reporting, ordered operator actions, persisted operator notes/metadata, reviewable `requested_changes`, and Admin Dashboard runbook/snapshot/config request visibility without adapter import/call, prompt submission, queue read/submission, upload, generation, runtime switch enablement, config mutation, or secret resolution.
