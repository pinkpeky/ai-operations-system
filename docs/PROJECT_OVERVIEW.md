...
The current next branch is `codex/phase-62a-comfyui-runtime-adapter-contract`, scoped to Phase 62A ComfyUI Runtime Adapter Contract: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `ComfyUIRuntimeService`, explicit `COMFYUI_RUNTIME_*` settings, Docker environment exposure, docs/runtime verification, and a dedicated Admin Dashboard ComfyUI tab contract panel before any real ComfyUI adapter import, HTTP request, prompt submission, queue read, file upload, queue submission, generation, runtime switch enablement, config mutation, or secret resolution.
