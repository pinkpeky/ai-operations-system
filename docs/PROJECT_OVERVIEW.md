...
The current next branch is `codex/phase-67e-digital-human-output-ingestion` for Phase 67E Digital Human Output Ingestion. It builds on the Phase 67A digital-human foundation, Phase 67B approved execution bridge, Phase 67C workflow binding, and Phase 67D workflow readiness by adding `/api/v1/digital-humans/video-jobs/{job_id}/comfyui-output-ingestion`. The ingestion layer refreshes the linked ComfyUI video job, reads recorded output files, and registers them as a generated digital-human `video` asset with `comfyui_output_ingestion_status`, `delivery_asset_id`, `delivery_asset_status`, `delivery_source_uri`, and `delivery_output_count`. Admin Dashboard Digital Humans now shows a concise output-ingestion panel, while `worker_console` and `worker_console_desktop` show customer-machine digital-human video progress plus output-ingestion and delivery status. Defaults remain safe with `DIGITAL_HUMAN_PROVIDER=mock`, `DIGITAL_HUMAN_ENABLED=false`, and `DIGITAL_HUMAN_ALLOW_EXTERNAL_API=false`; ingestion refreshes ComfyUI history by default but does not resubmit jobs unless `resubmit_if_waiting` is explicitly enabled. This phase does not call HeyGen/Tavus/D-ID/local avatar models by default, upload files to ComfyUI automatically, download models, resolve secrets, install workflows, publish to social media, control real accounts, mutate runtime configuration, restart services, bypass approval, run OpenClaw/Playwright publishing, ingest platform analytics, or rebuild client packages.

Phase 67E is stacked after Phase 67D Digital Human Workflow Readiness on `codex/phase-67d-digital-human-workflow-readiness`, Phase 67C Digital Human Workflow Binding on `codex/phase-67c-digital-human-workflow-binding`, Phase 67B Digital Human Execution Loop on `codex/phase-67b-digital-human-execution-loop`, Phase 67A Digital Human Foundation on `codex/phase-67a-digital-human-foundation`, Phase 66B ComfyUI Video Job Loop on `codex/phase-66b-comfyui-video-job-loop`, Phase 66A ComfyUI Video Resource Line on `codex/phase-66a-comfyui-video-resource-line`, Phase 65B Commercial ComfyUI Execution Link on `codex/phase-65b-commercial-comfyui-execution-link`, Phase 65A ComfyUI Real Adapter on `codex/phase-65a-comfyui-real-adapter`, Phase 64E Layout Declutter on `codex/phase-64e-layout-declutter`, Phase 64D Server/Client Frontend Operability Optimization on `codex/phase-64d-frontend-operability-optimization`, Phase 64C Commercial Agent/Skill Orchestration on `codex/phase-64c-commercial-agent-skill-orchestration`, Phase 63X-64B Customer Console Closed Loop Delivery Pass on `codex/phase-63x-64b-client-closed-loop-delivery`, Phase 62Y Commercial Operation Loop Protocol on `codex/phase-62y-operation-loop-protocol`, and the earlier Phase 62K-62W customer-console simplification/knowledge slices. The immediate operator surfaces now include the Admin Dashboard ComfyUI page with real queue/smoke prompt visibility, video resource planning, and video job status/refresh controls; a separate Admin Dashboard Digital Humans page for portrait/material upload, script-based video jobs, workflow template binding, real-workflow readiness, ComfyUI output ingestion, local delivery assets, and guarded ComfyUI handoff; the Commercial Operations ComfyUI dispatch panel with Submit ComfyUI / Refresh output actions; plus the server maintenance cockpit and simplified customer-machine `worker_console` / `worker_console_desktop` operation loop with digital-human video, workflow readiness, output-ingestion, and delivery status.

Phase 64E remains documented as the UI predecessor whose explicit goal was to remove the duplicate closed-loop delivery panel while moving support detail into `client-operation-support-drawer` and raw server output into `commercial-action-result-drawer`.

Phase 62J still inherits the Phase 62H ComfyUI Runtime Post-Manual Readiness Checks boundary for guarded probe approval, including `manual_config_applied`, `api_config_mutation_performed`, `guarded_probe_ready`, `health_probe_executed`, `external_request_attempted`, and `probe_result_status` audit fields. Phase 65A extends this guarded line with `prompt_submission_enabled`, `runtime_calls_enabled`, `prompt_id`, `node_errors`, queue status, and history outputs while keeping default runtime calls disabled. Phase 65B then stores those runtime prompt/output fields on commercial dispatches and asset requests after approval. Phase 66A adds `admission_status`, `should_submit_now`, `selected_endpoint`, `endpoint_plans`, `selected_gpu`, GPU VRAM estimates, queue counts, and `runtime_base_url` before any video prompt can be submitted. Phase 66B adds `video_job_count`, `job_status`, `runtime_prompt_id`, `outputs`, `failure_reason`, and `result_summary` as the recoverable video job view. Phase 67A adds `asset_count`, `provider_calls_enabled`, `consent_status`, `approval_status`, and digital-human provider boundary metadata for portrait/material/video-job planning. Phase 67B adds `progress_percent`, `current_stage`, `next_action`, `linked_comfyui_video_job_id`, and `DIGITAL_HUMAN_OUTPUT_DIR` so approved digital-human jobs can move from plan/review into recoverable delivery or guarded ComfyUI video handoff. Phase 67C adds `selected_workflow_template_id`, `workflow_binding_status`, `liveportrait-musetalk-broll`, required node/model contracts, and workflow-bound prompt/workflow metadata before any real graph submission. Phase 67D adds `workflow_readiness_status`, `workflow_asset_upload_status`, `workflow_output_watch_status`, `workflow_missing_nodes`, `workflow_missing_models`, upload manifest evidence, and GPU readiness evidence before any real prompt can be submitted. Phase 67E adds `comfyui_output_ingestion_status`, `delivery_asset_id`, `delivery_asset_status`, `delivery_source_uri`, and `delivery_output_count` so real ComfyUI outputs become reviewable digital-human delivery assets.

Customer console phase anchors for `worker_console` and `worker_console_desktop`: Phase 62K Customer Console Codex-like UX Simplification (`codex/phase-62k-customer-console-codex-ux`), Phase 62L Customer Console Task Workbench (`codex/phase-62l-client-task-workbench`), Phase 62M Customer Console Goal Templates (`codex/phase-62m-client-goal-templates`), Phase 62N Customer Console Goal Plan Preview (`codex/phase-62n-client-goal-plan-preview`), Phase 62O Customer Console Goal Status Tracker (`codex/phase-62o-client-goal-status-tracker`), Phase 62P Customer Console Simple Operator Mode (`codex/phase-62p-client-simple-operator-mode`) with the knowledge base upload/edit page, Phase 62Q Customer Console Knowledge Upload Readiness (`codex/phase-62q-knowledge-upload-readiness`) with knowledge upload readiness, Phase 62R Customer Console Knowledge Activity Timeline (`codex/phase-62r-knowledge-activity-timeline`) with knowledge activity timeline, Phase 62S Customer Console Knowledge Document Details (`codex/phase-62s-knowledge-document-details`) with knowledge document details, Phase 62T Customer Console Knowledge Search Validation (`codex/phase-62t-knowledge-search-validation`) with knowledge search validation, Phase 62U Customer Console Knowledge Ingestion Status Loop (`codex/phase-62u-knowledge-ingestion-status`) with knowledge ingestion status loop, Phase 62V Customer Console Knowledge Validation Guidance (`codex/phase-62v-knowledge-validation-guidance`) with knowledge validation guidance, Phase 62W Customer Console Knowledge Validation Outcomes (`codex/phase-62w-knowledge-validation-outcomes`) with knowledge validation outcomes, Phase 62X Customer Console Product Operation Desk (`codex/phase-62x-client-operation-desk`) with the product operation desk, Phase 63A Customer Console Loop Protocol Binding (`codex/phase-63a-client-loop-protocol-binding`) with operation-loop binding, Phase 63B Customer Console First Draft Bootstrap (`codex/phase-63b-client-first-draft-bootstrap`) with first draft approval bootstrap, Phase 63C Customer Console Approval and Execution Prep (`codex/phase-63c-client-approval-execution-prep`) with approval and metadata-only execution prep, Phase 63D Customer Console Execution Run Review (`codex/phase-63d-client-execution-run-review`) with execution run review, Phase 63E Customer Console Result Feedback Loop (`codex/phase-63e-client-result-feedback-loop`) with the minimum usable closed loop, Phase 63F Customer Console Next-Cycle Content Drafts (`codex/phase-63f-next-cycle-content-drafts`) with next-cycle content draft generation, Phase 63G Customer Console Next-Cycle Execution Prep (`codex/phase-63g-next-cycle-execution-prep`) with next-cycle execution prep, Phase 63H Customer Console Next-Cycle Execution Run Review (`codex/phase-63h-next-cycle-execution-run-review`) with next-cycle execution run review, Phase 63I Customer Console Next-Cycle Result Feedback Loop (`codex/phase-63i-next-cycle-result-feedback-loop`) with next-cycle result feedback, Phase 63J Customer Console Client Runtime Preflight (`codex/phase-63j-client-runtime-preflight`) with client runtime preflight, and Phase 63K Customer Console Guarded Adapter Dispatch Handoff (`codex/phase-63k-guarded-adapter-dispatch-handoff`) with guarded adapter dispatch handoff.
...
- Commercial Operations Foundation with `commercial_operations`, `CommercialOperationService`, `/api/v1/commercial-operations`, and a bilingual Admin Dashboard Commercial Ops page.
- Commercial Operation Evidence & Handoff Links with `commercial_operation_links`, create/list/delete link APIs, and Admin Dashboard controls for attaching conversations, artifacts, tasks, workflows, RAG documents, approvals, knowledge sources, and external materials to an operation.
- Commercial Operation Approval Gates with `commercial_operation_approvals`, create/list/approve/reject/cancel APIs, and Admin Dashboard controls for human approval decisions on individual operation plan steps.
- Commercial Operation Safe Dry-Runs with `commercial_operation_dry_runs`, create/list/complete/fail/cancel APIs, and Admin Dashboard controls for metadata-only execution preparation records created from approved approval gates.
- Commercial Operation Content Drafts with `commercial_operation_content_drafts`, create/list/update/ready/approve/reject/archive APIs, and Admin Dashboard controls for reviewable channel drafts.
- Commercial Operation Asset Requests with `commercial_operation_asset_requests`, create/list/update/ready/approve/reject/prepare/fail/archive APIs, and Admin Dashboard controls for first-class asset handoff preparation.
- Commercial Operation Deliverables with `commercial_operation_deliverables`, create/list/update/ready/approve/reject/package/fail/archive APIs, linked Output Library artifacts, and Admin Dashboard controls for approved commercial handoff packages.
- Commercial Operation Execution Requests with `commercial_operation_execution_requests`, create/list/update/ready/approve/reject/prepare/fail/cancel/archive APIs, and Admin Dashboard controls for metadata-only future runtime handoff requests.
- Commercial Operation Execution Runs with `commercial_operation_execution_runs`, create/list/update/start/succeed/fail/retry/cancel/archive APIs, and Admin Dashboard controls for metadata-only run audit and recovery records.
- Commercial Operation Results with `commercial_operation_results`, create/list/update/ready/approve/reject/archive APIs, and Admin Dashboard controls for operator-observed commercial result records without platform analytics ingestion or ROI attribution.
- Commercial Operation Monitoring Observations with `commercial_operation_monitoring_observations`, create/list/update/ready/approve/reject/archive APIs, and Admin Dashboard controls for operator-observed monitoring snapshots without platform analytics ingestion or ROI attribution.
- Commercial Operation Optimization Decisions with `commercial_operation_optimization_decisions`, create/list/update/ready/approve/reject/archive APIs, and Admin Dashboard controls for operator-decided next actions without automatic optimization, publishing, or external execution.
- Commercial Operation Evidence Snapshots with `commercial_operation_evidence_snapshots`, create/list/update/ready/approve/reject/archive APIs, execution request/run evidence snapshot IDs, operator checklists, and Admin Dashboard controls for reviewed knowledge/source evidence.
- Commercial Operation RAG Evidence Generation with `/evidence-snapshots/generate-rag`, existing RAG search, retrieved chunk evidence items, source document IDs, and draft-only snapshots without knowledge ingestion, approval bypass, publishing, or external execution.
- Commercial Operation RAG Content Draft Generation with `/content-drafts/generate-rag`, existing RAG search, retrieved chunk source materials, and draft-only content records without knowledge ingestion, approval bypass, publishing, or external execution.
- Commercial Operation RAG Asset Brief Generation with `/asset-requests/generate-rag`, existing RAG search, retrieved chunk source materials, readiness checks, and draft-only asset request records without knowledge ingestion, approval bypass, ComfyUI jobs, publishing, or external execution.
- Phase 63A Customer Console Loop Protocol Binding with `worker_console`, `worker_console_desktop`, commercial operation API clients, create-operation controls, `operation-loop` refresh, shared loop stage rendering, and local task-state fallback without OpenClaw/Playwright execution or approval bypass.
- Phase 63B Customer Console First Draft Bootstrap with `worker_console`, `worker_console_desktop`, plan regeneration, first content draft creation, ready-for-review transition, human approval gate creation, and `operation-loop` refresh without publishing or external execution.
- Phase 63C Customer Console Approval and Execution Prep with `worker_console`, `worker_console_desktop`, pending commercial approval approve/reject controls, linked content draft approval/rejection, deliverable packaging, and metadata-only OpenClaw/Playwright execution prep request creation without publishing or external execution.
- Phase 63D Customer Console Execution Run Review with `worker_console`, `worker_console_desktop`, execution request review and prepare controls, metadata-only execution run creation, start/failure recording, and retry controls without publishing or external execution.
- Phase 63E Customer Console Result Feedback Loop with `worker_console`, `worker_console_desktop`, the minimum usable closed loop from execution run to approved result, approved observation, and approved optimization decision without publishing, analytics ingestion, or external execution.
- Phase 63F Customer Console Next-Cycle Content Drafts with `worker_console`, `worker_console_desktop`, next-cycle content draft generation from an approved optimization decision, and a fresh human approval gate without publishing, analytics ingestion, or external execution.
- Phase 63G Customer Console Next-Cycle Execution Prep with `worker_console`, `worker_console_desktop`, next-cycle approval handling, next-cycle deliverable packaging, and metadata-only execution prep request creation without publishing, analytics ingestion, or external execution.
- Phase 63H Customer Console Next-Cycle Execution Run Review with `worker_console`, `worker_console_desktop`, next-cycle execution prep preference, queued metadata-only next-cycle execution runs, and existing start/retry/result controls without publishing, analytics ingestion, or external execution.
- Phase 63I Customer Console Next-Cycle Result Feedback Loop with `worker_console`, `worker_console_desktop`, next-cycle execution run preference, metadata-only result/observation/optimization records, and previous optimization lineage preservation without publishing, analytics ingestion, or external execution.
- Phase 63J Customer Console Client Runtime Preflight with `worker_console`, `worker_console_desktop`, client runtime preflight for queued/retrying execution runs, local Worker API health/status checks, and metadata-only `client_runtime_preflight` ready/blocked records without publishing, analytics ingestion, or external execution.
- Phase 63K Customer Console Guarded Adapter Dispatch Handoff with `worker_console`, `worker_console_desktop`, guarded adapter dispatch handoff for preflight-ready queued/retrying execution runs, metadata-only `guarded_adapter_dispatch_handoff` records, and explicit operator start requirement without publishing, analytics ingestion, or external execution.
- Commercial Operation ComfyUI Handoffs with `commercial_operation_comfyui_handoffs`, `/comfyui-handoffs`, prompt/workflow payload storage, readiness checks, review/approval/preparation lifecycle, and Admin Dashboard controls without submitting ComfyUI jobs, generating media, publishing, or external execution.
- Commercial Operation ComfyUI Preflights with `commercial_operation_comfyui_preflights`, `/comfyui-preflights`, endpoint/queue/model/workflow readiness records, optional adapter config selection, and Admin Dashboard controls without calling ComfyUI endpoints or submitting queues.
- Commercial Operation ComfyUI Adapter Configs with `commercial_operation_comfyui_adapter_configs`, `/comfyui-adapter-configs`, endpoint/queue/workflow allowlist/model inventory/runtime-limit/secret-reference validation, and Admin Dashboard controls without storing secret values, calling ComfyUI endpoints, or submitting queues.
- Commercial Operation ComfyUI Job Requests with `commercial_operation_comfyui_job_requests`, `/comfyui-job-requests`, checked-preflight-to-job-request review lifecycle, safety checks, output expectations, recovery guidance, and Admin Dashboard controls without calling ComfyUI endpoints, uploading files, submitting queues, or generating media.
- Commercial Operation ComfyUI Execution Plans with `commercial_operation_comfyui_execution_plans`, `/comfyui-execution-plans`, approved/queued-job-request-to-execution-plan review lifecycle, queue simulation payload normalization, operator checklist, rollback guidance, and Admin Dashboard controls without calling ComfyUI endpoints, uploading files, submitting queues, or generating media.
- Commercial Operation ComfyUI Connection Probes with `commercial_operation_comfyui_connection_probes`, `/comfyui-connection-probes`, approved/simulated-execution-plan-to-connection-probe review lifecycle, metadata-only health and queue snapshot planning, route/readiness normalization, and Admin Dashboard controls without calling ComfyUI endpoints, reading queues, uploading files, submitting queues, or generating media.
- Commercial Operation ComfyUI Adapter Dispatches with `commercial_operation_comfyui_adapter_dispatches`, `/comfyui-adapter-dispatches`, probed-connection-to-adapter-dispatch review lifecycle, metadata-only prompt/workflow/queue/dispatch payload planning, guardrails, retry/recovery metadata, and Admin Dashboard controls without calling ComfyUI endpoints, submitting prompts, reading queues, uploading files, submitting queues, or generating media.
- Commercial Operation ComfyUI Runtime Gates with `commercial_operation_comfyui_runtime_gates`, `/comfyui-runtime-gates`, dispatched-adapter-to-runtime-gate review lifecycle, metadata-only runtime switch, network/queue/secret/approval policies, validation checks, rollback guidance, and Admin Dashboard controls without enabling ComfyUI runtime calls, reading queues, submitting prompts or queues, uploading files, resolving secrets, or generating media.
- Commercial Operation ComfyUI Runtime Dry-Runs with `commercial_operation_comfyui_runtime_dry_runs`, `/comfyui-runtime-dry-runs`, armed-runtime-gate-to-runtime-dry-run review lifecycle, metadata-only adapter contract, request fixture, expected response contract, explicit server switch policy, validation checks, rollback guidance, and Admin Dashboard controls without importing or calling ComfyUI adapters, enabling runtime switches, reading queues, submitting prompts or queues, uploading files, resolving secrets, or generating media.
- Commercial Operation ComfyUI Runtime Activations with `commercial_operation_comfyui_runtime_activations`, `/comfyui-runtime-activations`, validated-runtime-dry-run-to-runtime-activation review lifecycle, metadata-only activation request records, switch audit records, runtime guardrails, validation checks, rollback guidance, and Admin Dashboard controls without importing or calling ComfyUI adapters, enabling runtime switches, reading queues, submitting prompts or queues, uploading files, resolving secrets, or generating media.
- ComfyUI Runtime Adapter Contract with `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `ComfyUIRuntimeService`, `COMFYUI_RUNTIME_*` settings, and the Admin Dashboard ComfyUI runtime adapter contract panel for disabled-by-default provider/config/allowlist/guardrail review without importing adapters, calling ComfyUI, reading queues, submitting prompts, uploading files, generating media, enabling runtime switches, mutating configuration, or resolving secrets.
- ComfyUI Guarded Read-Only Probe with `COMFYUI_RUNTIME_READ_ONLY_PROBE_ENABLED`, `COMFYUI_RUNTIME_HEALTH_PATH`, `COMFYUI_RUNTIME_ALLOWED_HEALTH_PATHS`, a gated `GET /system_stats` probe only when every explicit switch/allowlist is satisfied, and Admin Dashboard probe status fields without adapter import/call, prompt submission, queue read/submission, upload, generation, runtime switch enablement, config mutation, or secret resolution.
- ComfyUI Runtime Diagnostics, Maintenance Runbook, Diagnostic Snapshots, Configuration Change Requests, Manual Apply Evidence, Post-Manual Readiness Checks, Guarded Probe Execution Audit, Phase 65A Real Adapter, Phase 65B Commercial ComfyUI Execution Link, Phase 66A ComfyUI Video Resource Line, Phase 66B ComfyUI Video Job Loop, Phase 67A Digital Human Foundation, Phase 67B Digital Human Execution Loop, Phase 67C Digital Human Workflow Binding, and Phase 67D Digital Human Workflow Readiness with `/api/v1/comfyui-runtime/diagnostics`, `/api/v1/comfyui-runtime/maintenance-runbook`, `/api/v1/comfyui-runtime/diagnostic-snapshots`, `/api/v1/comfyui-runtime/config-change-requests`, `/api/v1/comfyui-runtime/manual-apply-evidence`, `/api/v1/comfyui-runtime/post-manual-readiness-checks`, `/api/v1/comfyui-runtime/guarded-probe-executions`, `/api/v1/comfyui-runtime/prompt-jobs`, `/api/v1/comfyui-runtime/video-resource-plans`, `/api/v1/comfyui-runtime/video-jobs`, `/api/v1/comfyui-runtime/video-jobs/{job_id}/refresh`, `/api/v1/digital-humans/capabilities`, `/api/v1/digital-humans/assets`, `/api/v1/digital-humans/workflow-templates`, `/api/v1/digital-humans/workflow-templates/{template_id}`, `/api/v1/digital-humans/video-jobs`, `/api/v1/digital-humans/video-jobs/{job_id}/workflow-binding`, `/api/v1/digital-humans/video-jobs/{job_id}/workflow-readiness-check`, `/api/v1/digital-humans/video-jobs/{job_id}/refresh`, `/api/v1/digital-humans/video-jobs/{job_id}/execute`, `/api/v1/digital-humans/video-jobs/{job_id}/{action}`, `/api/v1/comfyui-runtime/prompt-jobs/{prompt_id}/history`, `/api/v1/comfyui-runtime/queue`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/submit-runtime`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/refresh-runtime`, no-network readiness reporting, ordered operator actions, persisted operator notes/metadata, reviewable `requested_changes`, verified manual evidence, `comparison_status`, `guarded_probe_ready`, Admin Dashboard runbook/snapshot/config/evidence/readiness/probe-execution visibility, Admin Dashboard queue/smoke prompt/video-resource/video-job controls, Admin Dashboard Digital Humans upload/job/review/execute/bind/readiness controls, customer-machine digital-human video and workflow progress/readiness cards, and commercial dispatch Submit ComfyUI / Refresh output controls. The guarded prompt/queue/history/video admission calls require every explicit provider/network/host/path/prompt gate plus commercial approval; digital-human provider calls remain mock/disabled by default; secret resolution, publishing, account control, runtime switch mutation, config mutation, and approval bypass remain disabled.
- Phase 62I Workstation/Customer Client Frontend UX Alignment for `worker_console` and `worker_console_desktop`, adding a simplified operator home, local connection/runtime/heartbeat/recovery status cards, shortcuts for conversation/playbook/approval/output/task/log workflows, recovery guidance, setup/help copy, Chinese/English language switching, and explicit server/client boundary messaging without adding live ComfyUI, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha bypass, proxy pools, or fingerprint bypass.
- Phase 62O Customer Console Goal Status Tracker for `worker_console` and `worker_console_desktop`, adding a compact prepare/approval/execution/recovery/output tracker with current run status, thread/task ids, pending approvals, active tasks, failed/recoverable tasks, and artifacts without adding live ComfyUI, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha/proxy/fingerprint bypass, secret resolution, or approval bypass.
- Phase 62P Customer Console Simple Operator Mode for `worker_console` and `worker_console_desktop`, adding a compact goal input, common task chips, visual current progress, collapsed maintenance details, and a separate knowledge base upload/edit page with file upload, text add/update, upload queue, document cards, and refresh controls. The knowledge base upload/edit page is visual and does not display code or JSON. It does not add live ComfyUI, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha/proxy/fingerprint bypass, secret resolution, or approval bypass.
- Phase 62Q Customer Console Knowledge Upload Readiness for `worker_console` and `worker_console_desktop`, adding knowledge upload readiness cards, supported file/type and 20 MB size checks, next-step guidance, failed retry, queue item removal, and clear-completed controls on the separate visual knowledge base upload/edit page. The knowledge upload readiness flow does not display code or JSON and does not add live ComfyUI, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha/proxy/fingerprint bypass, secret resolution, or approval bypass.
- Phase 62R Customer Console Knowledge Activity Timeline for `worker_console` and `worker_console_desktop`, adding a visual recent activity timeline for selected file batches, upload success/failure counts, text saves, manual refreshes, queue item removal, and clear-completed actions on the separate visual knowledge base upload/edit page. The knowledge activity timeline does not display code or JSON and does not add live ComfyUI, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha/proxy/fingerprint bypass, secret resolution, or approval bypass.
- Phase 62S Customer Console Knowledge Document Details for `worker_console` and `worker_console_desktop`, adding a visual document processing overview, selected-document details, status cues, source id, collection, chunk count, timestamps, and direct update action on the separate visual knowledge base upload/edit page. The knowledge document details flow does not display code or JSON and does not add live ComfyUI, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha/proxy/fingerprint bypass, secret resolution, or approval bypass.
- Phase 62T Customer Console Knowledge Search Validation for `worker_console` and `worker_console_desktop`, adding a visual validation question, hybrid/semantic/keyword mode selector, existing RAG search call, result cards, source labels, chunk indexes, score summaries, empty-result guidance, and local activity records on the separate visual knowledge base upload/edit page. The knowledge search validation flow does not display code or JSON and does not add live ComfyUI, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha/proxy/fingerprint bypass, secret resolution, or approval bypass.
- Phase 62U Customer Console Knowledge Ingestion Status Loop for `worker_console` and `worker_console_desktop`, adding queued/uploading/processing/search-ready/needs-review counts, simple processing steps, latest upload result cards, source/document ids, chunk counts, duplicate-skip notes, failure reasons, refresh/retry actions, and selected-document ingestion status on the separate visual knowledge base upload/edit page. The knowledge ingestion status loop does not display code or JSON and does not add live ComfyUI, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha/proxy/fingerprint bypass, secret resolution, or approval bypass.
- Phase 62V Customer Console Knowledge Validation Guidance for `worker_console` and `worker_console_desktop`, adding suggested validation questions, selected-document and latest-upload targets, one-click fill/run actions, and source-scoped RAG search validation on the separate visual knowledge base upload/edit page. The knowledge validation guidance does not display code or JSON and does not add live ComfyUI, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha/proxy/fingerprint bypass, secret resolution, or approval bypass.
- Phase 62W Customer Console Knowledge Validation Outcomes for `worker_console` and `worker_console_desktop`, adding ready/needs-evidence/needs-review decisions, evidence counts, material context, validation mode, and next-step actions on the separate visual knowledge base upload/edit page. The knowledge validation outcomes do not display code or JSON and do not add live ComfyUI, OpenClaw execution, publishing, account control, installer signing, auto-update, captcha/proxy/fingerprint bypass, secret resolution, or approval bypass.
- Phase 62X Customer Console Product Operation Desk for `worker_console` and `worker_console_desktop`, adding a product operation desk with product/campaign topic input, process status, deliverables for copy/video/data analysis/operation direction, interrupt/continue controls, knowledge upload access, and explicit OpenClaw/Playwright client-execution positioning. The product operation desk does not execute OpenClaw, run Playwright, publish, control accounts, call ComfyUI, bypass captcha/proxy/fingerprint controls, resolve secrets, or bypass approval.
- Phase 62Y Commercial Operation Loop Protocol with `GET /api/v1/commercial-operations/{operation_id}/operation-loop`, `CommercialOperationLoopSummaryResponse`, `CommercialOperationLoopStageResponse`, current stage, next action, blocked reasons, record counts, server/customer frontend readiness, and future OpenClaw/Playwright execution protocol. The summary is read-only and does not execute OpenClaw, run Playwright, publish, control accounts, call ComfyUI, or bypass approval.
...
Phase 63O-63Q Customer Console Publish Result Observation Loop advances the first usable commercial automation loop in `worker_console` and `worker_console_desktop` from dry-run execution into visible publish/result/data tracking. It adds a guarded publish handoff, a manual publish result record, and a manual metric observation panel so an operator can continue the cycle after a succeeded dry-run without needing to inspect raw records.

## Phase 63R-63T Customer Console Publish Metric Improvement Loop

Branch: `codex/phase-63r-63t-publish-metric-improvement-loop`

Phase 63R-63T Customer Console Publish Metric Improvement Loop advances the first usable commercial automation loop in `worker_console` and `worker_console_desktop` from publish/result/data tracking into improvement and next-content preparation. It adds manual publish metric improvement, publish metric next-cycle draft preparation, and a visible content improvement step so an operator can move from recorded data into the next cycle without reading raw records.

It remains metadata/manual-record only: no live OpenClaw or Playwright execution, no social publishing, no account control, no platform analytics ingestion, no ComfyUI call, no automated optimization, and no approval bypass.
...
Phase 63U-63W Customer Console Improved Draft Re-execution Loop advances the first usable commercial automation loop in `worker_console` and `worker_console_desktop` from improved content back into customer-machine execution preparation. It adds publish metric approval recognition, publish metric re-execution prep, and improved draft re-execution run queueing so an operator can approve the improved draft and continue the next pass without raw record inspection.

It remains metadata/manual-record only: no live OpenClaw or Playwright execution, no social publishing, no account control, no platform analytics ingestion, no ComfyUI call, no automated optimization, and no approval bypass.

The customer-console UI now exposes a publish result loop panel with Prepare publish handoff, Capture publish result, and Record metrics actions. The records are `guarded_publish_handoff`, `manual_publish_result`, and `manual_publish_metrics`. Live OpenClaw execution, Playwright runs, social publishing, account control, automated platform analytics ingestion, ComfyUI calls, secret resolution, captcha/proxy/fingerprint bypass, and approval bypass remain out of scope.

## Phase 63X-64B Customer Console Closed Loop Delivery Pass

Branch: `codex/phase-63x-64b-client-closed-loop-delivery`

Phase 63X-64B Customer Console Closed Loop Delivery Pass advances the first usable commercial automation loop in `worker_console` and `worker_console_desktop` from separate execution, publishing, data, improvement, and re-execution controls into one client closed-loop delivery pass. The operator can run one guarded action that sequences client runtime preflight, OpenClaw/Playwright handoff, guarded dry-run, publish result capture, manual metric observation, improvement analysis, and next draft generation.

The customer-console UI presents the combined path as five human-readable steps: client execution prep, publish result capture, operating data observation, improvement analysis, and next draft generation. This keeps the first deliverable closed loop usable before deeper optimization work expands live integrations.
