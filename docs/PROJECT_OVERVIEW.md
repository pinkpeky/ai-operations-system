...
The current next branch is `codex/phase-61z-commercial-comfyui-runtime-activations`, scoped to Phase 61Z Commercial Operation ComfyUI Runtime Activations: `commercial_operation_comfyui_runtime_activations`, `/api/v1/commercial-operations/{operation_id}/comfyui-runtime-activations`, a dedicated Admin Dashboard ComfyUI tab with runtime activation controls, validated-runtime-dry-run-to-runtime-activation review lifecycle, metadata-only activation request records, switch audit records, runtime guardrails, validation checks, rollback guidance, and runtime activation state before any real ComfyUI adapter import, HTTP request, prompt submission, queue read, file upload, queue submission, generation, runtime switch enablement, or secret resolution.
