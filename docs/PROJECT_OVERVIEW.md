...
The current next branch is `codex/phase-65b-commercial-comfyui-execution-link` for Phase 65B Commercial ComfyUI Execution Link. It builds on Phase 65A by connecting approved commercial operation ComfyUI adapter dispatches to the guarded real ComfyUI runtime: `POST /api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/submit-runtime` submits the approved prompt graph, and `POST /api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/refresh-runtime` refreshes history/queue output state. Phase 65B records `runtime_prompt_id`, runtime submission/history/queue metadata, generated filenames, plan outline state, and prepared asset request handoff payloads. It does not upload files, download models, resolve secrets, publish to social media, control real accounts, mutate runtime configuration, restart services, bypass approval, run OpenClaw/Playwright publishing, ingest platform analytics, or rebuild client packages.

Phase 65B is stacked after Phase 65A ComfyUI Real Adapter on `codex/phase-65a-comfyui-real-adapter`, Phase 64E Layout Declutter on `codex/phase-64e-layout-declutter`, Phase 64D Server/Client Frontend Operability Optimization on `codex/phase-64d-frontend-operability-optimization`, Phase 64C Commercial Agent/Skill Orchestration on `codex/phase-64c-commercial-agent-skill-orchestration`, Phase 63X-64B Customer Console Closed Loop Delivery Pass on `codex/phase-63x-64b-client-closed-loop-delivery`, Phase 62Y Commercial Operation Loop Protocol on `codex/phase-62y-operation-loop-protocol`, and the earlier Phase 62K-62W customer-console simplification/knowledge slices. The immediate operator surfaces now include the Admin Dashboard ComfyUI page with real queue/smoke prompt visibility, the Commercial Operations ComfyUI dispatch panel with Submit ComfyUI / Refresh output actions, plus the server maintenance cockpit and simplified customer-machine `worker_console` / `worker_console_desktop` operation loop. Server-side ComfyUI prompt submission is now possible only through explicit gates and approved dispatch records; the customer console publish/result loop still creates manual and metadata-only handoff/result/metric/improvement/re-execution/closed-loop delivery records only.
...
- ComfyUI Runtime Diagnostics, Maintenance Runbook, Diagnostic Snapshots, Configuration Change Requests, Manual Apply Evidence, Post-Manual Readiness Checks, Guarded Probe Execution Audit, Phase 65A Real Adapter, and Phase 65B Commercial ComfyUI Execution Link with `/api/v1/comfyui-runtime/diagnostics`, `/api/v1/comfyui-runtime/maintenance-runbook`, `/api/v1/comfyui-runtime/diagnostic-snapshots`, `/api/v1/comfyui-runtime/config-change-requests`, `/api/v1/comfyui-runtime/manual-apply-evidence`, `/api/v1/comfyui-runtime/post-manual-readiness-checks`, `/api/v1/comfyui-runtime/guarded-probe-executions`, `/api/v1/comfyui-runtime/prompt-jobs`, `/api/v1/comfyui-runtime/prompt-jobs/{prompt_id}/history`, `/api/v1/comfyui-runtime/queue`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/submit-runtime`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/refresh-runtime`, no-network readiness reporting, ordered operator actions, persisted operator notes/metadata, reviewable `requested_changes`, verified manual evidence, `comparison_status`, `guarded_probe_ready`, Admin Dashboard runbook/snapshot/config/evidence/readiness/probe-execution visibility, Admin Dashboard queue/smoke prompt controls, and commercial dispatch Submit ComfyUI / Refresh output controls. The guarded prompt/queue/history calls require every explicit provider/network/host/path/prompt gate plus commercial approval; uploads, secret resolution, publishing, account control, runtime switch mutation, config mutation, and approval bypass remain disabled.
