...
The current next branch is `codex/phase-66b-comfyui-video-job-loop` for Phase 66B ComfyUI Video Job Loop. It builds on Phase 66A by turning guarded video GPU/queue admission into a persisted, recoverable job object: `POST /api/v1/comfyui-runtime/video-jobs` creates a job, reuses `/video-resource-plans`, submits real `/prompt` only when admitted, stores `runtime_prompt_id`, `runtime_base_url`, `resource_plan`, `selected_endpoint`, `selected_gpu`, queue/history payloads, output files, `job_status`, `failure_reason`, and `result_summary`, and `POST /api/v1/comfyui-runtime/video-jobs/{job_id}/refresh` refreshes progress. It does not upload files, download models, resolve secrets, install workflows, publish to social media, control real accounts, mutate runtime configuration, restart services, bypass approval, run OpenClaw/Playwright publishing, ingest platform analytics, or rebuild client packages.

Phase 66B is stacked after Phase 66A ComfyUI Video Resource Line on `codex/phase-66a-comfyui-video-resource-line`, Phase 65B Commercial ComfyUI Execution Link on `codex/phase-65b-commercial-comfyui-execution-link`, Phase 65A ComfyUI Real Adapter on `codex/phase-65a-comfyui-real-adapter`, Phase 64E Layout Declutter on `codex/phase-64e-layout-declutter`, Phase 64D Server/Client Frontend Operability Optimization on `codex/phase-64d-frontend-operability-optimization`, Phase 64C Commercial Agent/Skill Orchestration on `codex/phase-64c-commercial-agent-skill-orchestration`, Phase 63X-64B Customer Console Closed Loop Delivery Pass on `codex/phase-63x-64b-client-closed-loop-delivery`, Phase 62Y Commercial Operation Loop Protocol on `codex/phase-62y-operation-loop-protocol`, and the earlier Phase 62K-62W customer-console simplification/knowledge slices. The immediate operator surfaces now include the Admin Dashboard ComfyUI page with real queue/smoke prompt visibility, video resource planning, and video job status/refresh controls; the Commercial Operations ComfyUI dispatch panel with Submit ComfyUI / Refresh output actions; plus the server maintenance cockpit and simplified customer-machine `worker_console` / `worker_console_desktop` operation loop. Server-side ComfyUI prompt submission is now possible only through explicit gates, GPU/queue admission, persisted video job state, and approved dispatch records; the customer console publish/result loop still creates manual and metadata-only handoff/result/metric/improvement/re-execution/closed-loop delivery records only.
...
- ComfyUI Runtime Diagnostics, Maintenance Runbook, Diagnostic Snapshots, Configuration Change Requests, Manual Apply Evidence, Post-Manual Readiness Checks, Guarded Probe Execution Audit, Phase 65A Real Adapter, Phase 65B Commercial ComfyUI Execution Link, Phase 66A ComfyUI Video Resource Line, and Phase 66B ComfyUI Video Job Loop with `/api/v1/comfyui-runtime/diagnostics`, `/api/v1/comfyui-runtime/maintenance-runbook`, `/api/v1/comfyui-runtime/diagnostic-snapshots`, `/api/v1/comfyui-runtime/config-change-requests`, `/api/v1/comfyui-runtime/manual-apply-evidence`, `/api/v1/comfyui-runtime/post-manual-readiness-checks`, `/api/v1/comfyui-runtime/guarded-probe-executions`, `/api/v1/comfyui-runtime/prompt-jobs`, `/api/v1/comfyui-runtime/video-resource-plans`, `/api/v1/comfyui-runtime/video-jobs`, `/api/v1/comfyui-runtime/video-jobs/{job_id}/refresh`, `/api/v1/comfyui-runtime/prompt-jobs/{prompt_id}/history`, `/api/v1/comfyui-runtime/queue`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/submit-runtime`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/refresh-runtime`, no-network readiness reporting, ordered operator actions, persisted operator notes/metadata, reviewable `requested_changes`, verified manual evidence, `comparison_status`, `guarded_probe_ready`, Admin Dashboard runbook/snapshot/config/evidence/readiness/probe-execution visibility, Admin Dashboard queue/smoke prompt/video-resource/video-job controls, and commercial dispatch Submit ComfyUI / Refresh output controls. The guarded prompt/queue/history/video admission calls require every explicit provider/network/host/path/prompt gate plus commercial approval; uploads, secret resolution, publishing, account control, runtime switch mutation, config mutation, and approval bypass remain disabled.
