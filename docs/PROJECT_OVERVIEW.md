...
The current next branch is `codex/phase-67d-digital-human-workflow-readiness` for Phase 67D Digital Human Workflow Readiness. It builds on the Phase 67A digital-human foundation, Phase 67B approved execution bridge, and Phase 67C workflow binding by adding `/api/v1/digital-humans/video-jobs/{job_id}/workflow-readiness-check`. The readiness layer records imported real ComfyUI graph evidence, installed node/model evidence, uploaded bound assets, output watch path, GPU/queue state, `workflow_readiness_status`, `workflow_asset_upload_status`, `workflow_output_watch_status`, `workflow_missing_nodes`, and `workflow_missing_models`. Admin Dashboard Digital Humans now shows a concise real-workflow readiness panel, while `worker_console` and `worker_console_desktop` show customer-machine digital-human video progress plus workflow readiness status. Defaults remain safe with `DIGITAL_HUMAN_PROVIDER=mock`, `DIGITAL_HUMAN_ENABLED=false`, and `DIGITAL_HUMAN_ALLOW_EXTERNAL_API=false`; bound prompt submission is refused unless an operator supplies a real prompt, the existing guarded ComfyUI runtime gates allow it, and workflow readiness is complete. This phase does not call HeyGen/Tavus/D-ID/local avatar models by default, upload files to ComfyUI automatically, download models, resolve secrets, install workflows, publish to social media, control real accounts, mutate runtime configuration, restart services, bypass approval, run OpenClaw/Playwright publishing, ingest platform analytics, or rebuild client packages.

Phase 67D is stacked after Phase 67C Digital Human Workflow Binding on `codex/phase-67c-digital-human-workflow-binding`, Phase 67B Digital Human Execution Loop on `codex/phase-67b-digital-human-execution-loop`, Phase 67A Digital Human Foundation on `codex/phase-67a-digital-human-foundation`, Phase 66B ComfyUI Video Job Loop on `codex/phase-66b-comfyui-video-job-loop`, Phase 66A ComfyUI Video Resource Line on `codex/phase-66a-comfyui-video-resource-line`, Phase 65B Commercial ComfyUI Execution Link on `codex/phase-65b-commercial-comfyui-execution-link`, Phase 65A ComfyUI Real Adapter on `codex/phase-65a-comfyui-real-adapter`, Phase 64E Layout Declutter on `codex/phase-64e-layout-declutter`, Phase 64D Server/Client Frontend Operability Optimization on `codex/phase-64d-frontend-operability-optimization`, Phase 64C Commercial Agent/Skill Orchestration on `codex/phase-64c-commercial-agent-skill-orchestration`, Phase 63X-64B Customer Console Closed Loop Delivery Pass on `codex/phase-63x-64b-client-closed-loop-delivery`, Phase 62Y Commercial Operation Loop Protocol on `codex/phase-62y-operation-loop-protocol`, and the earlier Phase 62K-62W customer-console simplification/knowledge slices. The immediate operator surfaces now include the Admin Dashboard ComfyUI page with real queue/smoke prompt visibility, video resource planning, and video job status/refresh controls; a separate Admin Dashboard Digital Humans page for portrait/material upload, script-based video jobs, workflow template binding, real-workflow readiness, local delivery assets, and guarded ComfyUI handoff; the Commercial Operations ComfyUI dispatch panel with Submit ComfyUI / Refresh output actions; plus the server maintenance cockpit and simplified customer-machine `worker_console` / `worker_console_desktop` operation loop with digital-human video and workflow readiness status.
