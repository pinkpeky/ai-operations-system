...
The current next branch is `codex/phase-67b-digital-human-execution-loop` for Phase 67B Digital Human Execution Loop. It builds on the Phase 67A digital-human foundation by adding an approved execution bridge: `/api/v1/digital-humans/video-jobs/{job_id}/execute` can create a local delivery manifest asset under `DIGITAL_HUMAN_OUTPUT_DIR` or create a guarded ComfyUI video handoff linked to `/api/v1/comfyui-runtime/video-jobs`. Admin Dashboard Digital Humans now shows job progress, delivery asset creation, and ComfyUI handoff controls, while `worker_console` and `worker_console_desktop` show customer-machine digital-human video progress. Defaults remain safe with `DIGITAL_HUMAN_PROVIDER=mock`, `DIGITAL_HUMAN_ENABLED=false`, and `DIGITAL_HUMAN_ALLOW_EXTERNAL_API=false`; generated placeholder ComfyUI prompts are not submitted unless an operator supplies a real prompt and the existing guarded ComfyUI runtime gates allow it. This phase does not call HeyGen/Tavus/D-ID/local avatar models by default, download models, resolve secrets, install workflows, publish to social media, control real accounts, mutate runtime configuration, restart services, bypass approval, run OpenClaw/Playwright publishing, ingest platform analytics, or rebuild client packages.

Phase 67B is stacked after Phase 67A Digital Human Foundation on `codex/phase-67a-digital-human-foundation`, Phase 66B ComfyUI Video Job Loop on `codex/phase-66b-comfyui-video-job-loop`, Phase 66A ComfyUI Video Resource Line on `codex/phase-66a-comfyui-video-resource-line`, Phase 65B Commercial ComfyUI Execution Link on `codex/phase-65b-commercial-comfyui-execution-link`, Phase 65A ComfyUI Real Adapter on `codex/phase-65a-comfyui-real-adapter`, Phase 64E Layout Declutter on `codex/phase-64e-layout-declutter`, Phase 64D Server/Client Frontend Operability Optimization on `codex/phase-64d-frontend-operability-optimization`, Phase 64C Commercial Agent/Skill Orchestration on `codex/phase-64c-commercial-agent-skill-orchestration`, Phase 63X-64B Customer Console Closed Loop Delivery Pass on `codex/phase-63x-64b-client-closed-loop-delivery`, Phase 62Y Commercial Operation Loop Protocol on `codex/phase-62y-operation-loop-protocol`, and the earlier Phase 62K-62W customer-console simplification/knowledge slices. The immediate operator surfaces now include the Admin Dashboard ComfyUI page with real queue/smoke prompt visibility, video resource planning, and video job status/refresh controls; a separate Admin Dashboard Digital Humans page for portrait/material upload, script-based video jobs, local delivery assets, and guarded ComfyUI handoff; the Commercial Operations ComfyUI dispatch panel with Submit ComfyUI / Refresh output actions; plus the server maintenance cockpit and simplified customer-machine `worker_console` / `worker_console_desktop` operation loop with digital-human video progress.
...
- ComfyUI Runtime Diagnostics, Maintenance Runbook, Diagnostic Snapshots, Configuration Change Requests, Manual Apply Evidence, Post-Manual Readiness Checks, Guarded Probe Execution Audit, Phase 65A Real Adapter, Phase 65B Commercial ComfyUI Execution Link, Phase 66A ComfyUI Video Resource Line, Phase 66B ComfyUI Video Job Loop, Phase 67A Digital Human Foundation, and Phase 67B Digital Human Execution Loop with `/api/v1/comfyui-runtime/diagnostics`, `/api/v1/comfyui-runtime/maintenance-runbook`, `/api/v1/comfyui-runtime/diagnostic-snapshots`, `/api/v1/comfyui-runtime/config-change-requests`, `/api/v1/comfyui-runtime/manual-apply-evidence`, `/api/v1/comfyui-runtime/post-manual-readiness-checks`, `/api/v1/comfyui-runtime/guarded-probe-executions`, `/api/v1/comfyui-runtime/prompt-jobs`, `/api/v1/comfyui-runtime/video-resource-plans`, `/api/v1/comfyui-runtime/video-jobs`, `/api/v1/comfyui-runtime/video-jobs/{job_id}/refresh`, `/api/v1/digital-humans/capabilities`, `/api/v1/digital-humans/assets`, `/api/v1/digital-humans/video-jobs`, `/api/v1/digital-humans/video-jobs/{job_id}/refresh`, `/api/v1/digital-humans/video-jobs/{job_id}/execute`, `/api/v1/digital-humans/video-jobs/{job_id}/{action}`, `/api/v1/comfyui-runtime/prompt-jobs/{prompt_id}/history`, `/api/v1/comfyui-runtime/queue`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/submit-runtime`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/refresh-runtime`, no-network readiness reporting, ordered operator actions, persisted operator notes/metadata, reviewable `requested_changes`, verified manual evidence, `comparison_status`, `guarded_probe_ready`, Admin Dashboard runbook/snapshot/config/evidence/readiness/probe-execution visibility, Admin Dashboard queue/smoke prompt/video-resource/video-job controls, Admin Dashboard Digital Humans upload/job/review/execute controls, customer-machine digital-human video progress cards, and commercial dispatch Submit ComfyUI / Refresh output controls. The guarded prompt/queue/history/video admission calls require every explicit provider/network/host/path/prompt gate plus commercial approval; digital-human provider calls remain mock/disabled by default; secret resolution, publishing, account control, runtime switch mutation, config mutation, and approval bypass remain disabled.
