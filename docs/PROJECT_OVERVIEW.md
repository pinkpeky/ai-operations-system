...
The current next branch is `codex/phase-61w-commercial-comfyui-adapter-dispatches`, scoped to Phase 61W Commercial Operation ComfyUI Adapter Dispatches: `commercial_operation_comfyui_adapter_dispatches`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches`, a Commercial Ops ComfyUI adapter dispatch panel, probed-connection-to-adapter-dispatch review lifecycle, metadata-only prompt/workflow/queue/dispatch payload planning, guardrails, retry/recovery metadata, and dispatch state before any real ComfyUI HTTP request, prompt submission, queue read, file upload, queue submission, or generation.
