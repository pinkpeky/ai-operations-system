...
The current next branch is `codex/phase-62d-comfyui-runtime-diagnostic-snapshots`, scoped to Phase 62D ComfyUI Runtime Diagnostic Snapshots: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `/api/v1/comfyui-runtime/diagnostics`, `POST /api/v1/comfyui-runtime/diagnostic-snapshots`, `GET /api/v1/comfyui-runtime/diagnostic-snapshots`, `comfyui_runtime_diagnostic_snapshots`, `ComfyUIRuntimeService`, `readiness_status`, `blocking_reasons`, `recommended_actions`, `read_only_probe_ready`, operator notes, snapshot metadata, and a dedicated Admin Dashboard ComfyUI tab snapshot action. The diagnostic and snapshot endpoints remain no-network. The only permitted live request is still the Phase 62B `GET /system_stats` health probe after every explicit provider/enabled/network/host/path/read-only gate is true, before any real ComfyUI adapter import, execution endpoint call, prompt submission, queue read, file upload, queue submission, generation, runtime switch enablement, config mutation, or secret resolution.
...
- ComfyUI Runtime Diagnostics and Diagnostic Snapshots with `/api/v1/comfyui-runtime/diagnostics`, `/api/v1/comfyui-runtime/diagnostic-snapshots`, `comfyui_runtime_diagnostic_snapshots`, no-network readiness reporting, persisted operator notes/metadata, and Admin Dashboard snapshot visibility without adapter import/call, prompt submission, queue read/submission, upload, generation, runtime switch enablement, config mutation, or secret resolution.
