...
The current next branch is `codex/phase-61t-commercial-comfyui-job-requests`, scoped to Phase 61T Commercial Operation ComfyUI Job Requests: `commercial_operation_comfyui_job_requests`, `/api/v1/commercial-operations/{operation_id}/comfyui-job-requests`, a Commercial Ops ComfyUI job request panel, checked-preflight-to-job-request review lifecycle, safety checks, output expectations, and recovery guidance before any real ComfyUI connection, file upload, queue submission, or generation.
