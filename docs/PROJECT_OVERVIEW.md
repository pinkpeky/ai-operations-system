...
The current next branch is `codex/phase-61u-commercial-comfyui-execution-plans`, scoped to Phase 61U Commercial Operation ComfyUI Execution Plans: `commercial_operation_comfyui_execution_plans`, `/api/v1/commercial-operations/{operation_id}/comfyui-execution-plans`, a Commercial Ops ComfyUI execution plan panel, approved/queued-job-request-to-execution-plan review lifecycle, queue simulation payload normalization, operator checklist, and rollback guidance before any real ComfyUI connection, file upload, queue submission, or generation.
