...
The current next branch is `codex/phase-61y-commercial-comfyui-runtime-dry-runs`, scoped to Phase 61Y Commercial Operation ComfyUI Runtime Dry-Runs: `commercial_operation_comfyui_runtime_dry_runs`, `/api/v1/commercial-operations/{operation_id}/comfyui-runtime-dry-runs`, a Commercial Ops ComfyUI runtime dry-run panel, armed-runtime-gate-to-runtime-dry-run review lifecycle, metadata-only adapter contract, dry-run request fixture, expected response contract, explicit server switch policy, validation checks, rollback guidance, and runtime dry-run state before any real ComfyUI adapter import, HTTP request, prompt submission, queue read, file upload, queue submission, generation, runtime switch enablement, or secret resolution.
