...
The current next branch is `codex/phase-66a-comfyui-video-resource-line` for Phase 66A ComfyUI Video Resource Line. It builds on Phase 65B by adding guarded video GPU/queue admission: `POST /api/v1/comfyui-runtime/video-resource-plans` estimates required free VRAM from width, height, frames, fps, model profile, and reserve settings, reads guarded `/system_stats` and `/queue`, and returns `admission_status`, `should_submit_now`, `selected_endpoint`, `endpoint_plans`, `selected_gpu`, blockers, and recommendations. `COMFYUI_VIDEO_GPU_ENDPOINTS` lets maintainers define multiple GPU-pinned ComfyUI instances so admitted video prompts submit to the selected endpoint. Video `POST /api/v1/comfyui-runtime/prompt-jobs` requests and approved commercial video asset dispatches are blocked before real `/prompt` unless the resource plan is admitted. It does not upload files, download models, resolve secrets, publish to social media, control real accounts, mutate runtime configuration, restart services, bypass approval, run OpenClaw/Playwright publishing, ingest platform analytics, or rebuild client packages.

Phase 66A is stacked after Phase 65B Commercial ComfyUI Execution Link on `codex/phase-65b-commercial-comfyui-execution-link`, Phase 65A ComfyUI Real Adapter on `codex/phase-65a-comfyui-real-adapter`, Phase 64E Layout Declutter on `codex/phase-64e-layout-declutter`, Phase 64D Server/Client Frontend Operability Optimization on `codex/phase-64d-frontend-operability-optimization`, Phase 64C Commercial Agent/Skill Orchestration on `codex/phase-64c-commercial-agent-skill-orchestration`, Phase 63X-64B Customer Console Closed Loop Delivery Pass on `codex/phase-63x-64b-client-closed-loop-delivery`, Phase 62Y Commercial Operation Loop Protocol on `codex/phase-62y-operation-loop-protocol`, and the earlier Phase 62K-62W customer-console simplification/knowledge slices. The immediate operator surfaces now include the Admin Dashboard ComfyUI page with real queue/smoke prompt visibility and video resource planning, the Commercial Operations ComfyUI dispatch panel with Submit ComfyUI / Refresh output actions, plus the server maintenance cockpit and simplified customer-machine `worker_console` / `worker_console_desktop` operation loop. Server-side ComfyUI prompt submission is now possible only through explicit gates, GPU/queue admission for video jobs, and approved dispatch records; the customer console publish/result loop still creates manual and metadata-only handoff/result/metric/improvement/re-execution/closed-loop delivery records only.
