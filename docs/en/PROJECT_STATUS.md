...
Current update (2026-05-19): Phase 57A-59C and Phase 60A-60F have landed on `main` through PR #22-#39, covering Run Cockpit foundation, actions, operator controls, deep links, refresh UX, Playbooks and Output Library context, search density, workflow handoff, workflow focus/provenance, the frontend language foundation, the Overview role entry, the Conversations operator console, the RAG Documents knowledge console, RAG knowledge maintenance controls, and Replay Center workflow observability simplification. The active next branch is `codex/phase-60-rag-live-validation`, scoped to live RAG upload/search/debug/reingest/delete validation, a workstation/server-maintainer guide, and concise RAG operation-loop guidance.
