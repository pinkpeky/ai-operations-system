...
Current update (2026-05-22): the active next branch is `codex/phase-62m-client-goal-templates` and PR #79 draft for Phase 62M Customer Console Goal Templates after PR #78 / Phase 62L Customer Console Task Workbench. It adds standard launch content, RAG evidence, asset brief, and page report templates to the first `worker_console` and `worker_console_desktop` task workbench, so customer-machine operators can start from a clear operating goal and recommended playbook. It preserves local runtime/heartbeat controls, Chinese/English language switching, approvals, playbooks, outputs, tasks, logs, and server/client boundary warnings. It does not import ComfyUI adapters, call ComfyUI, submit prompts, read or submit queues, upload files, generate media, enable runtime switches, mutate runtime configuration, resolve secrets, publish, run OpenClaw, control accounts, or bypass approval.
...
Phase 62L Customer Console Task Workbench from `codex/phase-62l-client-task-workbench` remains the immediate customer-console UX predecessor. Phase 62K Customer Console Codex-like UX Simplification from `codex/phase-62k-customer-console-codex-ux` remains the earlier simplification predecessor, and Phase 62J ComfyUI Runtime Guarded Probe Execution Audit remains the immediate runtime predecessor: the server-side guarded probe is approval-gated, while Phase 62M keeps the customer-machine UI focused on template-assisted goal submission, approval handling, and task recovery before advanced diagnostics.
