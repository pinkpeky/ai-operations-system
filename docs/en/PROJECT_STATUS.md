...
Current update (2026-05-25): the active next branch is `codex/phase-65b-commercial-comfyui-execution-link` after draft PR #110 / Phase 65A ComfyUI Real Adapter. It adds Phase 65B Commercial ComfyUI Execution Link: approved commercial operation ComfyUI adapter dispatches can submit a real guarded ComfyUI prompt job, refresh history/queue output state, and write `runtime_prompt_id`, runtime submission/history/queue metadata, generated output filenames, and prepared asset request handoff payloads back into the commercial operation record. Defaults still require every Phase 65A runtime gate. It does not upload files, download models, resolve secrets, publish to social media, control real accounts, bypass approval, mutate runtime configuration, restart services, or rebuild client packages.

Active runtime surface: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `/api/v1/comfyui-runtime/diagnostics`, `/api/v1/comfyui-runtime/maintenance-runbook`, `/api/v1/comfyui-runtime/config-change-requests`, `/api/v1/comfyui-runtime/manual-apply-evidence`, `/api/v1/comfyui-runtime/post-manual-readiness-checks`, `/api/v1/comfyui-runtime/guarded-probe-executions`, `/api/v1/comfyui-runtime/diagnostic-snapshots`, `/api/v1/comfyui-runtime/prompt-jobs`, `/api/v1/comfyui-runtime/prompt-jobs/{prompt_id}/history`, `/api/v1/comfyui-runtime/queue`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/submit-runtime`, and `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/refresh-runtime`, with `api_config_mutation_performed=false`, `guarded_probe_ready`, `health_probe_executed`, `external_request_attempted`, `probe_result_status`, `runtime_prompt_id`, and `runtime_outputs` audit fields.

Phase 65A ComfyUI Real Adapter from `codex/phase-65a-comfyui-real-adapter` is the immediate runtime predecessor. Phase 64E remains the layout predecessor. Phase 64D remains the server/client frontend operability predecessor. Phase 64C remains the Agent/Skill orchestration predecessor. Phase 63X-64B remains the closed-loop delivery predecessor, and Phase 62Y remains the operation-loop protocol predecessor. Phase 62J remains the guarded probe execution anchor for approval-gated read-only probes. Phase 65B binds approved commercial asset dispatches to guarded ComfyUI prompt/queue/history calls while keeping OpenClaw/Playwright publishing, account control, analytics ingestion, uploads, and approval bypass out of scope.
