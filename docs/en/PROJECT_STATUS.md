...
Current update (2026-05-22): the active next branch is `codex/phase-62r-knowledge-activity-timeline` with a draft PR pending after draft PR #83 / Phase 62Q Customer Console Knowledge Upload Readiness. It adds Phase 62R Customer Console Knowledge Activity Timeline for `worker_console` and `worker_console_desktop`: recent selected file batches, upload success/failure counts, text material saves, manual refreshes, queue item removal, and clear-completed records on the separate visual knowledge base upload/edit page. The knowledge activity timeline does not show code or JSON. It preserves local runtime/heartbeat controls, Chinese/English language switching, approvals, playbooks, outputs, tasks, logs, and server/client boundary warnings. It does not import ComfyUI adapters, call ComfyUI, submit prompts, read or submit queues, generate media, enable runtime switches, mutate runtime configuration, resolve secrets, publish, run OpenClaw, control accounts, or bypass approval.
