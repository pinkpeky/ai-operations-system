...
Current update (2026-05-25): the active next branch is `codex/phase-67c-digital-human-workflow-binding` after Phase 67B Digital Human Execution Loop. It adds Phase 67C Digital Human Workflow Binding: `/api/v1/digital-humans/workflow-templates`, `/api/v1/digital-humans/workflow-templates/{template_id}`, and `/api/v1/digital-humans/video-jobs/{job_id}/workflow-binding` expose ComfyUI workflow contracts such as `liveportrait-musetalk-broll`, bind authorized portrait/material inputs, record node/model/resource requirements, and pass a guarded prompt/workflow contract into `/api/v1/comfyui-runtime/video-jobs`. The Admin Dashboard `digital-humans` page now shows workflow template selection and binding controls; `worker_console` and `worker_console_desktop` show a compact digital-human video progress card with workflow binding status for customer-machine operators. Defaults remain mock/disabled with `DIGITAL_HUMAN_PROVIDER=mock`, `DIGITAL_HUMAN_ENABLED=false`, and `DIGITAL_HUMAN_ALLOW_EXTERNAL_API=false`, and ComfyUI prompt submission still requires an operator-supplied real prompt plus the existing guarded runtime gates. It does not call HeyGen/Tavus/D-ID/local avatar models by default, publish to social media, control real accounts, bypass approval, mutate runtime configuration, restart services, upload files to ComfyUI, download models, install workflows, or rebuild client packages.

Active runtime surface: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `/api/v1/comfyui-runtime/diagnostics`, `/api/v1/comfyui-runtime/maintenance-runbook`, `/api/v1/comfyui-runtime/config-change-requests`, `/api/v1/comfyui-runtime/manual-apply-evidence`, `/api/v1/comfyui-runtime/post-manual-readiness-checks`, `/api/v1/comfyui-runtime/guarded-probe-executions`, `/api/v1/comfyui-runtime/diagnostic-snapshots`, `/api/v1/comfyui-runtime/prompt-jobs`, `/api/v1/comfyui-runtime/video-resource-plans`, `/api/v1/comfyui-runtime/video-jobs`, `/api/v1/comfyui-runtime/video-jobs/{job_id}/refresh`, `/api/v1/comfyui-runtime/prompt-jobs/{prompt_id}/history`, `/api/v1/comfyui-runtime/queue`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/submit-runtime`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/refresh-runtime`, `/api/v1/digital-humans/capabilities`, `/api/v1/digital-humans/assets`, `/api/v1/digital-humans/assets/{asset_id}`, `/api/v1/digital-humans/workflow-templates`, `/api/v1/digital-humans/workflow-templates/{template_id}`, `/api/v1/digital-humans/video-jobs`, `/api/v1/digital-humans/video-jobs/{job_id}`, `/api/v1/digital-humans/video-jobs/{job_id}/workflow-binding`, `/api/v1/digital-humans/video-jobs/{job_id}/refresh`, `/api/v1/digital-humans/video-jobs/{job_id}/execute`, and `/api/v1/digital-humans/video-jobs/{job_id}/{action}`, with `api_config_mutation_performed=false`, `guarded_probe_ready`, `health_probe_executed`, `external_request_attempted`, `probe_result_status`, `prompt_submission_enabled`, `runtime_calls_enabled`, `runtime_prompt_id`, `runtime_base_url`, `runtime_outputs`, `admission_status`, `should_submit_now`, `selected_endpoint`, `endpoint_plans`, `selected_gpu`, `video_job_count`, `job_status`, `outputs`, `asset_count`, `provider_calls_enabled`, `consent_status`, `approval_status`, `progress_percent`, `linked_comfyui_video_job_id`, `selected_workflow_template_id`, `workflow_binding_status`, and `DIGITAL_HUMAN_OUTPUT_DIR` audit fields.

Phase 67B Digital Human Execution Loop from `codex/phase-67b-digital-human-execution-loop` is the immediate runtime predecessor. Phase 67A remains the asset/approval predecessor, Phase 66B remains the ComfyUI video job predecessor, Phase 66A remains the GPU/queue admission predecessor, Phase 65B remains the commercial runtime predecessor, Phase 65A remains the guarded real adapter predecessor, Phase 64E remains the layout predecessor, Phase 64D remains the server/client frontend operability predecessor, Phase 64C remains the Agent/Skill orchestration predecessor, Phase 63X-64B remains the closed-loop delivery predecessor, and Phase 62Y remains the operation-loop protocol predecessor. Phase 67C prioritizes real ComfyUI workflow binding while keeping live provider calls, OpenClaw/Playwright publishing, account control, analytics ingestion, workflow installation, model download, ComfyUI file upload, and approval bypass out of scope.
