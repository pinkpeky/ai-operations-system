...
Current update (2026-05-22): Phase 57A-59C, Phase 60A-60G, and Phase 61A-61P have landed on `main` through PR #22-#56. PR #57 through PR #70 are stacked draft ComfyUI/commercial-operation slices through Phase 62D. The active next branch is `codex/phase-62e-comfyui-maintenance-console`, scoped to Phase 62E ComfyUI Runtime Maintenance Runbook: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `/api/v1/comfyui-runtime/diagnostics`, `/api/v1/comfyui-runtime/maintenance-runbook`, `POST /api/v1/comfyui-runtime/diagnostic-snapshots`, `GET /api/v1/comfyui-runtime/diagnostic-snapshots`, `ComfyUIRuntimeService`, `readiness_status`, `blocking_reasons`, `recommended_actions`, `next_operator_action`, `recovery_actions`, `configuration_summary`, `snapshot_recommended`, and a dedicated Admin Dashboard ComfyUI tab runbook table. Diagnostics, runbook, and snapshots remain no-network; the only permitted live request is still the guarded `GET /system_stats` health probe after every explicit provider/enabled/network/host/path/read-only gate is true, before any real ComfyUI adapter import, execution endpoint call, prompt submission, queue read, file upload, queue submission, generation, runtime switch enablement, config mutation, or secret resolution.
