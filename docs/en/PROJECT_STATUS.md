...
Current update (2026-05-22): the active next branch is `codex/phase-62j-comfyui-guarded-probe-executions` and PR #76 draft for Phase 62J ComfyUI Runtime Guarded Probe Execution Audit after Phase 62I Workstation/Customer Client Frontend UX Alignment from `codex/phase-62i-workstation-client-ux`. It adds `comfyui_runtime_guarded_probe_executions`, guarded probe create/list/review APIs, Admin Dashboard ComfyUI page controls at `?page=comfyui-operations`, and a separate execute endpoint for the existing guarded read-only `/system_stats` health path. Create/list/review remain no-network; the Admin Dashboard page auto-loads diagnostics instead of `/health`, so refresh does not run a probe. Execute is allowed only after explicit approval and a fresh diagnostics readiness check. It does not import ComfyUI adapters, submit prompts, read or submit queues, upload files, generate media, enable runtime switches, mutate runtime configuration, resolve secrets, publish, run OpenClaw, control accounts, or bypass approval.
...
Phase 62I Workstation/Customer Client Frontend UX Alignment remains the immediate UX predecessor: `worker_console` and `worker_console_desktop` provide operator home status, runtime/heartbeat controls, shortcuts, recovery guidance, setup/help panels, Chinese/English language switching, and server/client boundary warnings before server-side guarded probe escalation.
