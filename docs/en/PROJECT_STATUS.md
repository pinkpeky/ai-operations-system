...
Current update (2026-05-21): Phase 57A-59C, Phase 60A-60G, and Phase 61A-61P have landed on `main` through PR #22-#56. PR #57 is the draft Phase 61Q ComfyUI handoff slice, PR #58 is the draft Phase 61R ComfyUI preflight slice, PR #59 is the draft Phase 61S ComfyUI adapter config slice, PR #60 is the draft Phase 61T ComfyUI job request slice, PR #61 is the draft Phase 61U ComfyUI execution plan slice, and PR #62 is the draft Phase 61V ComfyUI connection probe slice. The active next branch is `codex/phase-61w-commercial-comfyui-adapter-dispatches`, scoped to Phase 61W Commercial Operation ComfyUI Adapter Dispatches: `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches`, Admin Dashboard ComfyUI adapter dispatch controls, probed-connection-to-dispatch review lifecycle, metadata-only prompt/workflow/queue/dispatch payload planning, guardrails, retry/recovery metadata, and dispatch state before any real ComfyUI HTTP request, prompt submission, queue read, file upload, queue submission, or generation.
