...
Current update (2026-05-24): the active next branch is `codex/phase-64d-frontend-operability-optimization` after draft PR #107 / Phase 64C Commercial Agent/Skill Orchestration. It adds Phase 64D Server/Client Frontend Operability Optimization: `admin_dashboard` gains a server maintenance cockpit for connection, customer-machine visibility, selected operation, and Agent/Skill next-action status, while `worker_console` and `worker_console_desktop` show common commercial operations actions first and fold advanced execution/recovery controls. This makes the repeatable closed loop clearer for server maintainers and customer-machine operators before future real adapter work. It does not execute live OpenClaw, run live Playwright publishing, publish to social media, control real accounts, call ComfyUI, ingest platform analytics, enable runtime switches, mutate runtime configuration, resolve secrets, bypass captcha, use proxy pools, bypass fingerprints, bypass approval, or rebuild client packages.
...
Phase 64C Commercial Agent/Skill Orchestration from `codex/phase-64c-commercial-agent-skill-orchestration` is the immediate UI/API predecessor, and Phase 64D optimizes the server and customer-machine frontends on top of that Agent/Skill visibility. Phase 63X-64B remains the closed-loop delivery predecessor, Phase 63U-63W remains the improved draft re-execution predecessor, Phase 63R-63T remains the publish metric improvement predecessor, Phase 63O-63Q remains the publish/result/data predecessor, Phase 63L-63N remains the dry-run and approval predecessor, and Phase 62Y remains the operation-loop protocol predecessor. Phase 62J remains the immediate runtime predecessor: the server-side guarded probe is approval-gated, while Phase 64D keeps OpenClaw/Playwright, publishing, and analytics as manual or metadata-only customer-console records.
