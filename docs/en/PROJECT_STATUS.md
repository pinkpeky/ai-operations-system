...
Current update (2026-05-24): the active next branch is `codex/phase-63h-next-cycle-execution-run-review` after draft PR #98 / Phase 63G Customer Console Next-Cycle Execution Prep and is now draft PR #99. It adds Phase 63H Customer Console Next-Cycle Execution Run Review: `worker_console` and `worker_console_desktop` can prefer next-cycle execution prep requests, review them, create metadata-only next-cycle execution runs, and leave those runs queued for explicit operator start, retry, and result feedback. This moves the repeatable closed loop from second execution prep into next-cycle execution run tracking. It does not execute OpenClaw, run Playwright, publish to social media, control real accounts, call ComfyUI, ingest platform analytics, enable runtime switches, mutate runtime configuration, resolve secrets, bypass captcha, use proxy pools, bypass fingerprints, or bypass approval.
