...
Current update (2026-05-19): Phase 57A-59C, Phase 60A-60G, and Phase 61A have landed on `main` through PR #22-#41, covering Run Cockpit foundation, actions, operator controls, deep links, refresh UX, Playbooks and Output Library context, search density, workflow handoff, workflow focus/provenance, the frontend language foundation, the Overview role entry, the Conversations operator console, the RAG Documents knowledge console, RAG knowledge maintenance controls, Replay Center workflow observability simplification, RAG live validation guidance, and the Commercial Ops project center. The active next branch is `codex/phase-61b-commercial-operation-links`, scoped to Phase 61B Commercial Operation Evidence & Handoff Links: `commercial_operation_links`, `/api/v1/commercial-operations/{operation_id}/links`, Admin Dashboard evidence/handoff controls, and manual references to conversations, artifacts, task runs, workflow runs, RAG documents, approvals, knowledge sources, and external materials.
