...
Current update (2026-05-19): Phase 57A-59C, Phase 60A-60G, Phase 61A, Phase 61B, and Phase 61C have landed on `main` through PR #22-#43, covering Run Cockpit foundation, actions, operator controls, deep links, refresh UX, Playbooks and Output Library context, search density, workflow handoff, workflow focus/provenance, the frontend language foundation, the Overview role entry, the Conversations operator console, the RAG Documents knowledge console, RAG knowledge maintenance controls, Replay Center workflow observability simplification, RAG live validation guidance, the Commercial Ops project center, commercial operation evidence/handoff links, and commercial operation approval gates. The active next branch is `codex/phase-61d-commercial-operation-dry-runs`, scoped to Phase 61D Commercial Operation Safe Dry-Runs: `commercial_operation_dry_runs`, `/api/v1/commercial-operations/{operation_id}/dry-runs`, Admin Dashboard safe dry-run controls, and explicit create/complete/fail/cancel records for approved operation approval gates.
