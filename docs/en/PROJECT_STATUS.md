...
Current update (2026-05-22): the active next branch is `codex/phase-62q-knowledge-upload-readiness` with a draft PR pending after draft PR #82 / Phase 62P Customer Console Simple Operator Mode. It adds Phase 62Q Customer Console Knowledge Upload Readiness for `worker_console` and `worker_console_desktop`: knowledge upload readiness cards, pre-upload file rules, supported file/type checks, 20 MB file-size validation, next-step guidance, failed retry controls, individual queue item removal, and clear-completed controls on the separate visual knowledge base upload/edit page. The knowledge upload readiness flow does not show code or JSON. It preserves local runtime/heartbeat controls, Chinese/English language switching, approvals, playbooks, outputs, tasks, logs, and server/client boundary warnings. It does not import ComfyUI adapters, call ComfyUI, submit prompts, read or submit queues, generate media, enable runtime switches, mutate runtime configuration, resolve secrets, publish, run OpenClaw, control accounts, or bypass approval.
