...
Current update (2026-05-25): the active next branch is `codex/phase-65a-comfyui-real-adapter` after draft PR #109 / Phase 64E Layout Declutter. It adds Phase 65A ComfyUI Real Adapter: guarded real ComfyUI prompt submission and status reads through `POST /api/v1/comfyui-runtime/prompt-jobs`, `GET /api/v1/comfyui-runtime/prompt-jobs/{prompt_id}/history`, and `GET /api/v1/comfyui-runtime/queue`; explicit prompt/execution path gates; Admin Dashboard ComfyUI queue and smoke prompt controls; Docker/.env exposure; and a local real ComfyUI smoke verification. Defaults stay disabled. It does not upload files, resolve secrets, publish to social media, control real accounts, bypass approval, mutate runtime configuration, restart services, or rebuild client packages.
...
Phase 64E Layout Declutter from `codex/phase-64e-layout-declutter` is the immediate UI predecessor. Phase 64D remains the server/client frontend operability predecessor. Phase 64C remains the Agent/Skill orchestration predecessor. Phase 63X-64B remains the closed-loop delivery predecessor, and Phase 62Y remains the operation-loop protocol predecessor. Phase 62J remains the immediate runtime predecessor for approval-gated read-only probes; Phase 65A adds the first guarded real ComfyUI prompt/queue/history adapter while keeping OpenClaw/Playwright publishing, account control, analytics ingestion, uploads, and approvals out of scope.
