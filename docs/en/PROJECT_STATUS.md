...
Current update (2026-05-25): the active next branch is `codex/phase-66b-comfyui-video-job-loop` after Phase 66A ComfyUI Video Resource Line. It adds Phase 66B ComfyUI Video Job Loop: maintainers can create persisted video jobs through `POST /api/v1/comfyui-runtime/video-jobs`, reuse Phase 66A GPU/queue admission before real `/prompt`, store `runtime_prompt_id`, `runtime_base_url`, `resource_plan`, `selected_endpoint`, `selected_gpu`, queue/history payloads, output files, `job_status`, `failure_reason`, and `result_summary`, then recover progress through `POST /api/v1/comfyui-runtime/video-jobs/{job_id}/refresh`. Defaults still require every Phase 65A/65B runtime gate plus the Phase 66A video resource limits. It does not upload files, download models, resolve secrets, publish to social media, control real accounts, bypass approval, mutate runtime configuration, restart services, or rebuild client packages.

Active runtime surface: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `/api/v1/comfyui-runtime/diagnostics`, `/api/v1/comfyui-runtime/maintenance-runbook`, `/api/v1/comfyui-runtime/config-change-requests`, `/api/v1/comfyui-runtime/manual-apply-evidence`, `/api/v1/comfyui-runtime/post-manual-readiness-checks`, `/api/v1/comfyui-runtime/guarded-probe-executions`, `/api/v1/comfyui-runtime/diagnostic-snapshots`, `/api/v1/comfyui-runtime/prompt-jobs`, `/api/v1/comfyui-runtime/video-resource-plans`, `/api/v1/comfyui-runtime/video-jobs`, `/api/v1/comfyui-runtime/video-jobs/{job_id}/refresh`, `/api/v1/comfyui-runtime/prompt-jobs/{prompt_id}/history`, `/api/v1/comfyui-runtime/queue`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/submit-runtime`, and `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/refresh-runtime`, with `api_config_mutation_performed=false`, `guarded_probe_ready`, `health_probe_executed`, `external_request_attempted`, `probe_result_status`, `prompt_submission_enabled`, `runtime_calls_enabled`, `runtime_prompt_id`, `runtime_base_url`, `runtime_outputs`, `admission_status`, `should_submit_now`, `selected_endpoint`, `endpoint_plans`, `selected_gpu`, `video_job_count`, `job_status`, and `outputs` audit fields.

Phase 66A ComfyUI Video Resource Line from `codex/phase-66a-comfyui-video-resource-line` is the immediate runtime predecessor. Phase 65B remains the commercial runtime predecessor. Phase 65A remains the guarded real adapter predecessor. Phase 64E remains the layout predecessor. Phase 64D remains the server/client frontend operability predecessor. Phase 64C remains the Agent/Skill orchestration predecessor. Phase 63X-64B remains the closed-loop delivery predecessor, and Phase 62Y remains the operation-loop protocol predecessor. Phase 62J remains the guarded probe execution anchor for approval-gated read-only probes. Phase 66B prioritizes a recoverable video generation job object while keeping OpenClaw/Playwright publishing, account control, analytics ingestion, uploads, workflow installation, and approval bypass out of scope.
