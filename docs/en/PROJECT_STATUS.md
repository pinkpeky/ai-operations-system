...
Current update (2026-05-24): the active next branch is `codex/phase-63j-client-runtime-preflight` after draft PR #99 / Phase 63I Customer Console Next-Cycle Result Feedback Loop and is now draft PR #100. It adds Phase 63J Customer Console Client Runtime Preflight: `worker_console` and `worker_console_desktop` can prefer queued/retrying execution runs, run a client runtime preflight, read local Worker API health/status, record runtime/heartbeat/OpenClaw/browser readiness, and patch the execution run with metadata-only `client_runtime_preflight` ready/blocked details. This moves the repeatable closed loop toward an explicit customer-machine readiness gate before future guarded adapter work. It does not execute OpenClaw, run Playwright, publish to social media, control real accounts, call ComfyUI, ingest platform analytics, enable runtime switches, mutate runtime configuration, resolve secrets, bypass captcha, use proxy pools, bypass fingerprints, or bypass approval.
