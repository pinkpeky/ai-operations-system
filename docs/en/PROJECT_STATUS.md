...
Current update (2026-05-22): the active next branch is `codex/phase-62k-customer-console-codex-ux` and PR #77 draft for Phase 62K Customer Console Codex-like UX Simplification after PR #76 / Phase 62J ComfyUI Runtime Guarded Probe Execution Audit. It refactors `worker_console` and `worker_console_desktop` so customer-machine operators see a concise command surface first: status rail, next-step guidance, first-screen conversation input, neutral operational styling, and collapsed advanced maintenance/diagnostics. It preserves local runtime/heartbeat controls, Chinese/English language switching, approvals, playbooks, outputs, tasks, logs, and server/client boundary warnings. It does not import ComfyUI adapters, call ComfyUI, submit prompts, read or submit queues, upload files, generate media, enable runtime switches, mutate runtime configuration, resolve secrets, publish, run OpenClaw, control accounts, or bypass approval.
...
Phase 62I Workstation/Customer Client Frontend UX Alignment from `codex/phase-62i-workstation-client-ux` remains the immediate customer-console UX predecessor, and Phase 62J ComfyUI Runtime Guarded Probe Execution Audit remains the immediate runtime predecessor: the server-side guarded probe is approval-gated, while Phase 62K keeps the customer-machine UI focused on simple operation before advanced diagnostics.
