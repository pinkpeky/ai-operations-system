...
Current update (2026-05-22): the active next branch is `codex/phase-62v-knowledge-validation-guidance` with draft PR #88 after draft PR #87 / Phase 62U Customer Console Knowledge Ingestion Status Loop. It adds Phase 62V Customer Console Knowledge Validation Guidance for `worker_console` and `worker_console_desktop`: suggested validation questions, selected-document validation, latest-upload validation, one-click question fill/run actions, and source-scoped RAG search validation on the separate visual knowledge base upload/edit page. The knowledge validation guidance does not show code or JSON. It preserves local runtime/heartbeat controls, Chinese/English language switching, approvals, playbooks, outputs, tasks, logs, and server/client boundary warnings. It does not import ComfyUI adapters, call ComfyUI, submit prompts, read or submit queues beyond existing RAG search, generate media, enable runtime switches, mutate runtime configuration, resolve secrets, publish, run OpenClaw, control accounts, or bypass approval.
