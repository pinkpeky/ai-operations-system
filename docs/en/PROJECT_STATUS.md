...
Current update (2026-05-22): Phase 57A-59C, Phase 60A-60G, and Phase 61A-61P have landed on `main` through PR #22-#56. PR #57 through PR #71 are stacked draft ComfyUI/commercial-operation slices through Phase 62E. The active next branch is `codex/phase-62f-comfyui-config-change-requests`, scoped to Phase 62F ComfyUI Runtime Configuration Change Requests: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `/api/v1/comfyui-runtime/diagnostics`, `/api/v1/comfyui-runtime/maintenance-runbook`, `POST /api/v1/comfyui-runtime/config-change-requests`, `GET /api/v1/comfyui-runtime/config-change-requests`, ready/approve/reject/cancel/archive review actions, `POST /api/v1/comfyui-runtime/diagnostic-snapshots`, `GET /api/v1/comfyui-runtime/diagnostic-snapshots`, `comfyui_runtime_config_change_requests`, `ComfyUIRuntimeConfigChangeRequest`, `ComfyUIRuntimeService`, `readiness_status`, `blocking_reasons`, `recommended_actions`, `next_operator_action`, `recovery_actions`, `configuration_summary`, `snapshot_recommended`, `change_status`, `requested_changes`, `config_mutation_performed=false`, and a dedicated Admin Dashboard ComfyUI tab request table. Diagnostics, runbook, snapshots, and config change requests remain no-network; the only permitted live request is still the guarded `GET /system_stats` health probe after every explicit provider/enabled/network/host/path/read-only gate is true, before any real ComfyUI adapter import, execution endpoint call, prompt submission, queue read, file upload, queue submission, generation, runtime switch enablement, config mutation, or secret resolution.
