...
Current update (2026-05-22): Phase 57A-59C, Phase 60A-60G, and Phase 61A-61P have landed on `main` through PR #22-#56. PR #57 through PR #68 are stacked draft ComfyUI/commercial-operation slices through Phase 62B. The active next branch is `codex/phase-62c-comfyui-runtime-diagnostics`, scoped to Phase 62C ComfyUI Runtime Diagnostics: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `/api/v1/comfyui-runtime/diagnostics`, `ComfyUIRuntimeService`, `readiness_status`, `blocking_reasons`, `recommended_actions`, `read_only_probe_ready`, and a dedicated Admin Dashboard ComfyUI tab diagnostics panel. The diagnostics endpoint remains no-network; the only permitted live request is still the guarded `GET /system_stats` health probe after every explicit provider/enabled/network/host/path/read-only gate is true, before any real ComfyUI adapter import, execution endpoint call, prompt submission, queue read, file upload, queue submission, generation, runtime switch enablement, config mutation, or secret resolution.
