...
Current update (2026-05-19): Phase 57A-59C and Phase 60A-60E have landed on `main` through PR #22-#38, covering Run Cockpit foundation, actions, operator controls, deep links, refresh UX, Playbooks and Output Library context, search density, workflow handoff, workflow focus/provenance, the frontend language foundation, the Overview role entry, the Conversations operator console, the RAG Documents knowledge console, and RAG knowledge maintenance controls. The active next branch is `codex/phase-60-workflow-observability-simplification`, scoped to Replay Center command-center guidance, localized workflow observability labels, attention metrics, trace filtering, and clearer replay boundaries.
