...
Current update (2026-05-25): the active next branch is `codex/phase-64e-layout-declutter` after draft PR #108 / Phase 64D Server/Client Frontend Operability Optimization. It adds Phase 64E Layout Declutter: `worker_console` and `worker_console_desktop` remove the duplicate closed-loop delivery panel, fold deliverables, execution queue, and publish result controls into `client-operation-support-drawer`, and keep the common action strip as the primary operator path. `admin_dashboard` moves raw action result JSON into `commercial-action-result-drawer`. This makes the repeatable closed loop clearer for server maintainers and customer-machine operators before future real adapter work. It does not execute live OpenClaw, run live Playwright publishing, publish to social media, control real accounts, call ComfyUI, ingest platform analytics, enable runtime switches, mutate runtime configuration, resolve secrets, bypass captcha, use proxy pools, bypass fingerprints, bypass approval, or rebuild client packages.
...
Phase 64D Server/Client Frontend Operability Optimization from `codex/phase-64d-frontend-operability-optimization` is the immediate UI predecessor, and Phase 64E reduces visible page clutter on top of that simplified layout. Phase 64C remains the Agent/Skill orchestration predecessor. Phase 63X-64B remains the closed-loop delivery predecessor, Phase 63U-63W remains the improved draft re-execution predecessor, Phase 63R-63T remains the publish metric improvement predecessor, Phase 63O-63Q remains the publish/result/data predecessor, Phase 63L-63N remains the dry-run and approval predecessor, and Phase 62Y remains the operation-loop protocol predecessor. Phase 62J remains the immediate runtime predecessor: the server-side guarded probe is approval-gated, while Phase 64E keeps OpenClaw/Playwright, publishing, and analytics as manual or metadata-only customer-console records.
...
- Phase 64E Layout Declutter on `codex/phase-64e-layout-declutter`, including removal of the duplicate closed-loop delivery panel, `client-operation-support-drawer`, and `commercial-action-result-drawer`.
