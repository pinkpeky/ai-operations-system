...
Current update (2026-05-22): the active next branch is `codex/phase-62i-workstation-client-ux`, with PR #75 open as a draft for Phase 62I Workstation/Customer Client Frontend UX Alignment. `worker_console` and `worker_console_desktop` now add a simplified workstation/customer operator home, local connection/runtime/heartbeat/recovery status cards, runtime and heartbeat controls, conversation/playbook/approval/output/task/log shortcuts, setup/help panels, recovery guidance, Chinese/English language switching, and explicit server-vs-customer-machine boundary warnings. This remains a frontend/readiness slice and does not add live ComfyUI calls, OpenClaw execution, platform publishing, account control, installer signing, auto-update, captcha bypass, proxy pools, fingerprint bypass, secret resolution, or approval bypass.
