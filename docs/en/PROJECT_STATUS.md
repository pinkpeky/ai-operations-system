...
Current update (2026-05-25): the active next branch is `codex/phase-66a-comfyui-video-resource-line` after draft PR #111 / Phase 65B Commercial ComfyUI Execution Link. It adds Phase 66A ComfyUI Video Resource Line: maintainers can request a guarded video resource plan through `POST /api/v1/comfyui-runtime/video-resource-plans`, video prompt submissions are blocked unless GPU VRAM and queue admission pass, and approved commercial ComfyUI adapter dispatches route video asset requests through the same gate before real `/prompt` submission. `COMFYUI_VIDEO_GPU_ENDPOINTS` can define multiple GPU-pinned ComfyUI instances, and the runtime selects `selected_endpoint` before submitting video prompts. Defaults still require every Phase 65A/65B runtime gate plus the `COMFYUI_VIDEO_*` resource limits. It does not upload files, download models, resolve secrets, publish to social media, control real accounts, bypass approval, mutate runtime configuration, restart services, or rebuild client packages.

Active runtime surface: `/api/v1/comfyui-runtime/health`, `/api/v1/comfyui-runtime/capabilities`, `/api/v1/comfyui-runtime/diagnostics`, `/api/v1/comfyui-runtime/maintenance-runbook`, `/api/v1/comfyui-runtime/config-change-requests`, `/api/v1/comfyui-runtime/manual-apply-evidence`, `/api/v1/comfyui-runtime/post-manual-readiness-checks`, `/api/v1/comfyui-runtime/guarded-probe-executions`, `/api/v1/comfyui-runtime/diagnostic-snapshots`, `/api/v1/comfyui-runtime/prompt-jobs`, `/api/v1/comfyui-runtime/video-resource-plans`, `/api/v1/comfyui-runtime/prompt-jobs/{prompt_id}/history`, `/api/v1/comfyui-runtime/queue`, `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/submit-runtime`, and `/api/v1/commercial-operations/{operation_id}/comfyui-adapter-dispatches/{adapter_dispatch_id}/refresh-runtime`, with `api_config_mutation_performed=false`, `guarded_probe_ready`, `health_probe_executed`, `external_request_attempted`, `probe_result_status`, `prompt_submission_enabled`, `runtime_calls_enabled`, `runtime_prompt_id`, `runtime_base_url`, `runtime_outputs`, `admission_status`, `should_submit_now`, `selected_endpoint`, `endpoint_plans`, and `selected_gpu` audit fields.

Phase 65B Commercial ComfyUI Execution Link from `codex/phase-65b-commercial-comfyui-execution-link` is the immediate runtime predecessor. Phase 65A remains the guarded real adapter predecessor. Phase 64E remains the layout predecessor. Phase 64D remains the server/client frontend operability predecessor. Phase 64C remains the Agent/Skill orchestration predecessor. Phase 63X-64B remains the closed-loop delivery predecessor, and Phase 62Y remains the operation-loop protocol predecessor. Phase 62J remains the guarded probe execution anchor for approval-gated read-only probes. Phase 66A prioritizes the video generation line by gating video prompts on GPU/queue capacity while keeping OpenClaw/Playwright publishing, account control, analytics ingestion, uploads, and approval bypass out of scope.
