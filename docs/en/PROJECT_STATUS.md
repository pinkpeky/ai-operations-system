...
Current update (2026-05-22): the active next branch is `codex/phase-62o-client-goal-status-tracker` and draft PR #81 for Phase 62O Customer Console Goal Status Tracker after PR #80 / Phase 62N Customer Console Goal Plan Preview. It adds a compact prepare/approval/execution/recovery/output tracker for the selected customer-machine goal in `worker_console` and `worker_console_desktop`, including current run status, thread/task ids, pending approvals, active tasks, failed/recoverable tasks, and artifacts. It preserves local runtime/heartbeat controls, Chinese/English language switching, approvals, playbooks, outputs, tasks, logs, and server/client boundary warnings. It does not import ComfyUI adapters, call ComfyUI, submit prompts, read or submit queues, upload files, generate media, enable runtime switches, mutate runtime configuration, resolve secrets, publish, run OpenClaw, control accounts, or bypass approval.
