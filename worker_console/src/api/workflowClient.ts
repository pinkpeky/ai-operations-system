...
export const workflowClient = {
  ...
  createReplay: (workflowRunId: string, settings?: ConversationSettings) =>
    requestJson<{ id: string; replay_status: string; metadata: Record<string, unknown> }>(
      ...
      { method: "POST", body: JSON.stringify({ replay_reason: "Replay metadata requested from Worker Console" }) },
      settings,
    ),
  pause: (workflowRunId: string, settings?: ConversationSettings) =>
    requestJson<WorkflowRun>(`/workflow-runs/${workflowRunId}/pause`, { method: "POST", body: JSON.stringify({ reason: "Worker Console pause" }) }, settings),
  resume: (workflowRunId: string, settings?: ConversationSettings) =>
    requestJson<WorkflowRun>(`/workflow-runs/${workflowRunId}/resume`, { method: "POST", body: JSON.stringify({ reason: "Worker Console resume" }) }, settings),
};
