...
export const conversationClient = {
  ...
  createThread: (title: string, settings?: ConversationSettings) =>
    requestJson<ConversationThread>("/conversations", {
      method: "POST",
      body: JSON.stringify({ title, metadata: { source: "worker_console" } }),
    }, settings),
  ...
  sendMessage: (threadId: string, content: string, settings?: ConversationSettings) =>
    requestJson<ConversationMessage>(`/conversations/${threadId}/messages`, {
      method: "POST",
      body: JSON.stringify({ role: "user", content, metadata: { source: "worker_console" } }),
    }, settings),
  ...
  approveApproval: (approvalId: string, reviewer_notes = "Approved from Worker Console.", settings?: ConversationSettings) =>
    requestJson<ConversationApproval>(`/conversation-approvals/${approvalId}/approve`, {
      method: "POST",
      body: JSON.stringify({ reviewer_notes }),
    }, settings),
  rejectApproval: (approvalId: string, reviewer_notes = "Rejected from Worker Console.", settings?: ConversationSettings) =>
    requestJson<ConversationApproval>(`/conversation-approvals/${approvalId}/reject`, {
      method: "POST",
      body: JSON.stringify({ reviewer_notes }),
    }, settings),
  cancelApproval: (approvalId: string, reviewer_notes = "Cancelled from Worker Console.", settings?: ConversationSettings) =>
