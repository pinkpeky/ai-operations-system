...
Revision ID: 0052_phase61u_comfyui_plans
Revises: 0051_phase61t_comfyui_jobs
...
revision = "0052_phase61u_comfyui_plans"
down_revision = "0051_phase61t_comfyui_jobs"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("execution_steps", sa.JSON(), nullable=False, server_default=_json_default("[]")),
        sa.Column("simulation_checks", sa.JSON(), nullable=False, server_default=_json_default("[]")),
        ...
        sa.Column("simulation_payload", sa.JSON(), nullable=False, server_default=_json_default("{}")),
        sa.Column("plan_payload", sa.JSON(), nullable=False, server_default=_json_default("{}")),
        ...
        sa.Column("simulated_by", sa.String(length=128), nullable=True),
    ...
    for column in (
        ...
        op.create_index(
            f"ix_commercial_op_comfyui_plans_{column}",
            "commercial_operation_comfyui_execution_plans",
            [column],
        )
...
def downgrade() -> None:
    op.drop_table("commercial_operation_comfyui_execution_plans")
