...
class CommercialOperationComfyUIRuntimeDryRun(IdTimestampMixin, Base):
    """Metadata-only ComfyUI runtime adapter dry-run validation record."""

    __tablename__ = "commercial_operation_comfyui_runtime_dry_runs"
    ...
    runtime_gate_id: Mapped[UUID] = mapped_column(
        ...
        index=True,
        nullable=False,
        comment="Armed ComfyUI runtime gate ID",
    )
    adapter_dispatch_id: Mapped[UUID] = mapped_column(
        ForeignKey("commercial_operation_comfyui_adapter_dispatches.id", ondelete="RESTRICT"),
        index=True,
    ...
    step_key: Mapped[str] = mapped_column(String(128), index=True, nullable=False, comment="Plan step key")
    title: Mapped[str] = mapped_column(String(255), nullable=False, comment="ComfyUI runtime dry-run title")
    dry_run_status: Mapped[str] = mapped_column(
        String(32),
        default=CommercialOperationComfyUIRuntimeDryRunStatus.DRAFT.value,
        index=True,
        nullable=False,
        comment="draft / ready_for_review / approved / validated / failed / cancelled / archived",
    )
    dry_run_mode: Mapped[str] = mapped_column(
        String(32),
        default="metadata_only",
        index=True,
        nullable=False,
    ...
    queue_name: Mapped[str | None] = mapped_column(String(128), nullable=True, comment="Future ComfyUI queue name")
    workflow_name: Mapped[str] = mapped_column(String(128), index=True, nullable=False, comment="ComfyUI workflow name")
    adapter_contract: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict, nullable=False, comment="Runtime adapter contract snapshot")
    dry_run_request: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict, nullable=False, comment="Dry-run request payload")
    expected_response: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict, nullable=False, comment="Expected response contract")
    runtime_policy: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict, nullable=False, comment="Runtime policy and server switch controls")
    validation_checks: Mapped[list[dict[str, Any]]] = mapped_column(JSON, default=list, nullable=False, comment="Runtime dry-run validation checks")
    operator_checklist: Mapped[list[str]] = mapped_column(JSON, default=list, nullable=False, comment="Human operator checklist")
    rollback_plan: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict, nullable=False, comment="Rollback and disable plan")
    dry_run_payload: Mapped[dict[str, Any]] = mapped_column(
        ...
        default=dict,
        nullable=False,
        comment="Non-executing ComfyUI runtime dry-run payload",
    )
    result_summary: Mapped[str | None] = mapped_column(Text, nullable=True, comment="Runtime dry-run result summary")
    failure_reason: Mapped[str | None] = mapped_column(Text, nullable=True, comment="Failure or blocker reason")
    reviewer_notes: Mapped[str | None] = mapped_column(Text, nullable=True, comment="Reviewer notes")
    planned_by: Mapped[str | None] = mapped_column(String(128), index=True, nullable=True, comment="Planner user ID")
    updated_by: Mapped[str | None] = mapped_column(String(128), index=True, nullable=True, comment="Last updater user ID")
