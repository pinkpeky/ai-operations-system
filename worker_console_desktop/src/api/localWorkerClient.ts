...
export const DEFAULT_LOCAL_WORKER_API = (import.meta.env.VITE_LOCAL_WORKER_API ?? "/local-worker").replace(
  /\/$/,
  "",
);
...
async function requestJson<T>(baseUrl: string, path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    throw new Error(`Worker API unreachable at ${baseUrl}${path}: ${detail}`);
  }
...
export function createLocalWorkerClient(baseUrl = DEFAULT_LOCAL_WORKER_API) {
  ...
  return {
    ...
    getMetricDispatchScheduler: () =>
      requestJson<MetricDispatchLocalSchedulerState>(normalizedBaseUrl, "/local/metric-dispatch-scheduler"),
    configureMetricDispatchScheduler: (payload: Record<string, unknown>) =>
      requestJson<MetricDispatchLocalSchedulerState>(normalizedBaseUrl, "/local/metric-dispatch-scheduler/configure", {
        method: "POST",
        body: JSON.stringify(payload),
      }),
    tickMetricDispatchScheduler: (force = false) =>
      requestJson<MetricDispatchLocalSchedulerState>(normalizedBaseUrl, "/local/metric-dispatch-scheduler/tick", {
        method: "POST",
        body: JSON.stringify({ force }),
      }),
    startMetricDispatchScheduler: () =>
      requestJson<MetricDispatchLocalSchedulerState>(normalizedBaseUrl, "/local/metric-dispatch-scheduler/start", {
        method: "POST",
      }),
    stopMetricDispatchScheduler: () =>
      requestJson<MetricDispatchLocalSchedulerState>(normalizedBaseUrl, "/local/metric-dispatch-scheduler/stop", {
        method: "POST",
      }),
    clearMetricDispatchScheduler: () =>
      requestJson<MetricDispatchLocalSchedulerState>(normalizedBaseUrl, "/local/metric-dispatch-scheduler/clear", {
        method: "POST",
      }),
    openClawHealth: () => requestJson<LocalWorkerOpenClawHealth>(normalizedBaseUrl, "/openclaw/health"),
    openClawCapabilities: () => requestJson<LocalWorkerOpenClawCapabilities>(normalizedBaseUrl, "/openclaw/capabilities"),
    openClawProviderDiagnostics: () =>
      requestJson<LocalWorkerOpenClawProviderDiagnostics>(normalizedBaseUrl, "/openclaw/provider-diagnostics"),
    executeOpenClawAction: (payload: {
      ...
      requestJson<LocalWorkerOpenClawActionResponse>(normalizedBaseUrl, "/openclaw/actions", {
        method: "POST",
        body: JSON.stringify(payload),
      }),
  };
