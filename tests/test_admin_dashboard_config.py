...
def test_admin_dashboard_env_example_documents_runtime_config() -> None:
    ...
    assert "VITE_AI_SERVER_API=http://localhost:8000" in env_example
    assert "VITE_WORKSPACE_ID=demo-workspace" in env_example
    assert "VITE_USER_ID=demo-user" in env_example
