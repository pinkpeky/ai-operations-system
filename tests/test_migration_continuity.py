...
def test_migration_continuity_parses_phase_61m_head() -> None:
    ...
    phase_61w = (
        ROOT / "alembic/versions/20260521_0054_phase61w_commercial_operation_comfyui_adapter_dispatches.py"
    ).read_text(encoding="utf-8")
    assert "revision = \"0054_phase61w_comfyui_dispatches\"" in phase_61w
    assert "down_revision = \"0053_phase61v_comfyui_probes\"" in phase_61w

    phase_61x = (
        ROOT / "alembic/versions/20260521_0055_phase61x_commercial_operation_comfyui_runtime_gates.py"
    ).read_text(encoding="utf-8")
    assert "revision = \"0055_phase61x_comfyui_gates\"" in phase_61x
    assert "down_revision = \"0054_phase61w_comfyui_dispatches\"" in phase_61x

    phase_61y = (
        ROOT / "alembic/versions/20260521_0056_phase61y_commercial_operation_comfyui_runtime_dry_runs.py"
    ).read_text(encoding="utf-8")
    assert "revision = \"0056_phase61y_comfyui_dryruns\"" in phase_61y
    assert "down_revision = \"0055_phase61x_comfyui_gates\"" in phase_61y

    phase_61z = (
        ROOT / "alembic/versions/20260521_0057_phase61z_commercial_operation_comfyui_runtime_activations.py"
    ).read_text(encoding="utf-8")
    assert "revision = \"0057_phase61z_comfyui_active\"" in phase_61z
    assert "down_revision = \"0056_phase61y_comfyui_dryruns\"" in phase_61z

    phase_62d = (
        ROOT / "alembic/versions/20260522_0058_phase62d_comfyui_runtime_diagnostic_snapshots.py"
    ).read_text(encoding="utf-8")
    assert "revision = \"0058_phase62d_comfyui_snaps\"" in phase_62d
    assert "down_revision = \"0057_phase61z_comfyui_active\"" in phase_62d

    phase_62f = (
        ROOT / "alembic/versions/20260522_0059_phase62f_comfyui_runtime_config_change_requests.py"
    ).read_text(encoding="utf-8")
    assert "revision = \"0059_phase62f_comfyui_cfgreq\"" in phase_62f
    assert "down_revision = \"0058_phase62d_comfyui_snaps\"" in phase_62f

    phase_62g = (
        ROOT / "alembic/versions/20260522_0060_phase62g_comfyui_runtime_manual_apply_evidence.py"
    ).read_text(encoding="utf-8")
